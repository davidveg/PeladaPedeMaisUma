...
export async function POST(request: Request) {
  ...
  const rateLimit = await beginLoginAttempt(request, "member", email);
  if (!rateLimit.allowed) return loginRateLimitResponse(rateLimit);
  ...
  await db().batch(administrator ? [
    ...
    db().prepare(`INSERT INTO member_sessions (id,member_account_id,expires_at,created_at) VALUES (?,?,?,?)`).bind(await hashOpaqueToken(id), account.id, expires.toISOString(), now.toISOString()),
...
export async function PUT(request: Request) {
  ...
  try {
    await db().batch([
      ...
      db().prepare(`INSERT INTO member_sessions (id,member_account_id,expires_at,created_at) VALUES (?,?,?,?)`).bind(await hashOpaqueToken(sessionId), id, expires.toISOString(), now.toISOString()),
    ]);
  } catch (error: any) {
...
export async function DELETE(request: Request) {
  ...
  if (memberToken) await db().prepare(`DELETE FROM member_sessions WHERE id IN (?,?)`).bind(await hashOpaqueToken(memberToken), memberToken).run();
