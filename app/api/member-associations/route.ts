...
export async function GET(request: Request) {
  ...
  await ensureDb();
  const rows = await db().prepare(`SELECT accounts.*,l.player_id,p.display_name,p.type,p.primary_position FROM (SELECT id,email,active,last_login_at,created_at,updated_at,0 promoted_from_member,'member' account_type FROM member_accounts UNION ALL SELECT id,email,active,last_login_at,created_at,updated_at,promoted_from_member,'administrator' account_type FROM administrators) accounts LEFT JOIN player_account_links l ON l.account_type=accounts.account_type AND l.account_id=accounts.id LEFT JOIN players p ON p.id=l.player_id ORDER BY accounts.created_at DESC`).all();
  return Response.json({ associations: rows.results.map((row: any) => ({ id: row.id, email: row.email, accountType: row.account_type, promotedFromMember: !!row.promoted_from_member, canDemote: row.account_type === "administrator" && !!row.promoted_from_member, playerId: row.player_id, playerName: row.display_name, playerType: row.type, primaryPosition: row.primary_position, active: !!row.active, lastLoginAt: row.last_login_at, createdAt: row.created_at, updatedAt: row.updated_at })) }, { headers: { "cache-control": "no-store" } });
}
