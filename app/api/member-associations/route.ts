...
export async function GET(request: Request) {
  ...
  return Response.json({ associations: rows.results.map((row: any) => ({ id: row.id, email: row.email, accountType: row.account_type, role: row.role || (row.account_type === "administrator" ? "administrator" : "member"), promotedFromMember: !!row.promoted_from_member, canDemote: row.account_type === "administrator" && !!row.promoted_from_member, playerId: row.player_id, playerName: row.display_name, playerType: row.type, primaryPosition: row.primary_position, active: !!row.active, lastLoginAt: row.last_login_at, createdAt: row.created_at, updatedAt: row.updated_at })) }, { headers: { "cache-control": "no-store" } });
}
