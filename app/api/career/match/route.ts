...
export async function POST(request:Request){
  ...
  try{const match=await createCareerMatch(String(payload.separationId||""),Number(payload.blueScore),Number(payload.yellowScore),admin.id,Array.isArray(payload.contributions)?payload.contributions:[]);return Response.json({match,votingUrl:`/votacao?token=${match?.votingToken}`},{status:201});}
