...
export async function PUT(request:Request){
  ...
  const config:CareerConfig={enabled:Boolean(payload.enabled),trackContributions:Boolean(payload.trackContributions),momentumMultiplier:Number(payload.momentumMultiplier),winnerBonus:Number(payload.winnerBonus),loserPenalty:Number(payload.loserPenalty),motmThird:Number(payload.motmThird),motmSecond:Number(payload.motmSecond),motmFirst:Number(payload.motmFirst),dotmThird:Number(payload.dotmThird),dotmSecond:Number(payload.dotmSecond),dotmFirst:Number(payload.dotmFirst),votingDays:Number(payload.votingDays)};
  if(!validateCareerConfig(config))return Response.json({error:"Revise os valores: o multiplicador deve ficar entre 0 e 5, bônus entre 0 e 1, ônus entre -1 e 0 e a votação entre 1 e 30 dias."},{status:400});
  ...
  await db().prepare(`UPDATE career_configuration SET enabled=?,track_contributions=?,momentum_multiplier=?,winner_bonus=?,loser_penalty=?,motm_third=?,motm_second=?,motm_first=?,dotm_third=?,dotm_second=?,dotm_first=?,voting_days=?,updated_at=? WHERE id=1`).bind(config.enabled?1:0,config.trackContributions?1:0,config.momentumMultiplier,config.winnerBonus,config.loserPenalty,config.motmThird,config.motmSecond,config.motmFirst,config.dotmThird,config.dotmSecond,config.dotmFirst,config.votingDays,now).run();
