...
export async function PUT(request:Request){
  ...
  const config:CareerConfig={enabled:Boolean(payload.enabled),momentumMultiplier:Number(payload.momentumMultiplier),winnerBonus:Number(payload.winnerBonus),loserPenalty:Number(payload.loserPenalty),motmThird:Number(payload.motmThird),motmSecond:Number(payload.motmSecond),motmFirst:Number(payload.motmFirst),dotmThird:Number(payload.dotmThird),dotmSecond:Number(payload.dotmSecond),dotmFirst:Number(payload.dotmFirst),votingDays:Number(payload.votingDays)};
