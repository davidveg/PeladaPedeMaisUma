...
function mapConfiguration(c: any, career: any) {
  return {
    ...
    resultMomentumMultiplier: Number(career?.result_momentum_multiplier ?? 1), momentumMultiplier: Number(career?.momentum_multiplier ?? 1), historicalLearningEnabled: Boolean(c.historical_learning_enabled ?? 0), maximumPositionDifference: c.maximum_position_difference, protectedTopPlayersPercentage: c.protected_top_players_percentage, defaultReserveCount: c.default_reserve_count, algorithmAttempts: c.algorithm_attempts,
  };
}
