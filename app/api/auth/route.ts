...
export async function POST(request:Request){
  ...
  if(isAdministrator){
    ...
    await db().batch([
      db().prepare(`INSERT INTO member_sessions (id,member_account_id,expires_at,created_at) VALUES (?,?,?,?)`).bind(await hashOpaqueToken(token),account.id,expires.toISOString(),now.toISOString()),
