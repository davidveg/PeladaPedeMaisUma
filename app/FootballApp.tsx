...
function PlayerDetail({ player, config, onClose }: any) {
  ...
  return <div className="modal-back" onClick={onClose}><div className={`player-card-modal tier-${tier}`} onClick={event=>event.stopPropagation()}><button className="close" onClick={onClose} aria-label="Fechar detalhes">×</button><div className="player-card"><div className="card-top"><div className="overall"><strong>{overall.toFixed(1)}</strong><span>{config?.cardTiersEnabled?`OVERALL · ${playerCardTierLabel(tier)}`:'OVERALL'}</span></div><div className="card-photo"><PlayerPhoto photoUrl={player.photoUrl} name={player.displayName} large /></div></div><div className="card-identity"><h2>{player.displayName}</h2>{player.fullName!==player.displayName&&<p>{player.fullName}</p>}</div><div className="card-role"><span><small>TIPO</small><b>{type}</b></span><span><small>POSIÇÃO</small><b>{player.primaryPosition}</b></span></div><div className="card-stats">{stats.map(stat=><span key={stat.label}><b>{Number(stat.value).toFixed(1)}</b><small>{stat.label}</small></span>)}<span><b>{(player.momentum??0)>0?'+':''}{(player.momentum??0).toFixed(1)}</b><small>MOMENTUM</small></span></div><div className={`card-career-stats ${config?.showContributions?'with-contributions':''}`} aria-label="Estatísticas de partidas confirmadas"><span><b>{careerStats.games}</b><small>JOGOS</small></span><span className="wins"><b>{careerStats.wins}</b><small>VITÓRIAS</small></span><span className="losses"><b>{careerStats.losses}</b><small>DERROTAS</small></span>{config?.showContributions&&<><span className="goals"><b>{careerStats.goals??0}</b><small>GOLS</small></span><span className="assists"><b>{careerStats.assists??0}</b><small>ASSISTÊNCIAS</small></span></>}</div>{player.notes&&<blockquote>{player.notes}</blockquote>}</div></div></div>;
}
