...
function BalanceMetrics({ delta }: any) { return <div className="metrics"><h3>Diferenças entre os times</h3><div><Metric label="Jogadores" value={delta?.players ?? 0} /><Metric label="Defensores" value={delta?.defenders ?? 0} /><Metric label="Meio-campistas" value={delta?.midfielders ?? 0} /><Metric label="Atacantes" value={delta?.attackers ?? 0} /><Metric label="Físico / Pos." value={(delta?.speed ?? 0).toFixed(1)} /><Metric label="Técnica / Def." value={(delta?.skill ?? 0).toFixed(1)} /><Metric label="Marcação / Pés" value={(delta?.marking ?? 0).toFixed(1)} /><Metric label="Tática / Segurança" value={(delta?.tacticalIntelligence ?? 0).toFixed(1)} /><Metric label="Comp. / Liderança" value={(delta?.competitiveness ?? 0).toFixed(1)} /><Metric label="Momentum" value={(delta?.momentum ?? 0).toFixed(1)} /><Metric label="Pontuação" value={(delta?.score ?? 0).toFixed(2)} /></div></div>; }
...
function PlayerDetail({ player, config, onClose }: any) {
  ...
  const stats=goalkeeper?[{label:"DEFESAS",value:player.skill},{label:"POSICIONAMENTO",value:player.goalkeeperPositioning??player.speed??3},{label:"JOGO COM OS PÉS",value:player.goalExit??player.marking??3},{label:"SEGURANÇA",value:player.goalkeeperSafety??3},{label:"LIDERANÇA",value:player.goalkeeperLeadership??3}]:[{label:"FÍSICO",value:player.speed},{label:"TÉCNICA",value:player.skill},{label:"MARCAÇÃO",value:player.marking??3},{label:"INTELIGÊNCIA TÁTICA",value:player.tacticalIntelligence??3},{label:"COMPETITIVIDADE",value:player.competitiveness??3}];
  const careerStats=player.careerStats??{games:0,wins:0,losses:0,goals:0,assists:0};
  return <div className="modal-back" onClick={onClose}><div className={`player-card-modal tier-${tier}`} onClick={event=>event.stopPropagation()}><button className="close" onClick={onClose} aria-label="Fechar detalhes">×</button><div className="player-card"><div className="card-top"><div className="overall"><strong>{overall.toFixed(1)}</strong><span className="card-overall-label">OVERALL</span>{config?.cardTiersEnabled&&<em className="player-tier-badge">{playerCardTierLabel(tier)}</em>}</div><div className="card-photo"><PlayerPhoto photoUrl={player.photoUrl} name={player.displayName} large /></div></div><div className="card-identity"><h2>{player.displayName}</h2>{player.fullName!==player.displayName&&<p>{player.fullName}</p>}</div><div className="card-role"><span><small>TIPO</small><b>{type}</b></span><span><small>POSIÇÃO</small><b>{player.primaryPosition}</b></span></div><div className="card-stats">{stats.map(stat=><span key={stat.label}><b>{Number(stat.value).toFixed(1)}</b><small>{stat.label}</small></span>)}<span><b>{(player.momentum??0)>0?'+':''}{(player.momentum??0).toFixed(1)}</b><small>MOMENTUM</small></span></div><div className={`card-career-stats ${config?.showContributions?'with-contributions':''}`} aria-label="Estatísticas de partidas confirmadas"><span><b>{careerStats.games}</b><small>JOGOS</small></span><span className="wins"><b>{careerStats.wins}</b><small>VITÓRIAS</small></span><span className="losses"><b>{careerStats.losses}</b><small>DERROTAS</small></span>{config?.showContributions&&<><span className="goals"><b>{careerStats.goals??0}</b><small>GOLS</small></span><span className="assists"><b>{careerStats.assists??0}</b><small>ASSISTÊNCIAS</small></span></>}</div>{player.notes&&<blockquote>{player.notes}</blockquote>}</div></div></div>;
}
