...
function resultConfig(result: any) {
  ...
  return { ...defaultConfig, speedWeight: result?.speedWeight ?? defaultConfig.speedWeight, skillWeight: result?.skillWeight ?? defaultConfig.skillWeight, markingWeight: result?.markingWeight ?? (legacySnapshot ? 0 : defaultConfig.markingWeight), resultMomentumMultiplier: result?.resultMomentumMultiplier ?? defaultConfig.resultMomentumMultiplier, momentumMultiplier: result?.momentumMultiplier ?? defaultConfig.momentumMultiplier, maximumPositionDifference: result?.maximumPositionDifference ?? defaultConfig.maximumPositionDifference, protectedTopPlayersPercentage: result?.protectedTopPlayersPercentage ?? defaultConfig.protectedTopPlayersPercentage, algorithmAttempts: result?.algorithmAttempts ?? defaultConfig.algorithmAttempts };
}
