...
function BalanceMetrics({ delta }: any) { return <div className="metrics"><h3>Diferenças entre os times</h3><div><Metric label="Jogadores" value={delta?.players ?? 0} /><Metric label="Defensores" value={delta?.defenders ?? 0} /><Metric label="Meio-campistas" value={delta?.midfielders ?? 0} /><Metric label="Atacantes" value={delta?.attackers ?? 0} /><Metric label="Físico / Pos." value={(delta?.speed ?? 0).toFixed(1)} /><Metric label="Técnica / Def." value={(delta?.skill ?? 0).toFixed(1)} /><Metric label="Marcação / Pés" value={(delta?.marking ?? 0).toFixed(1)} /><Metric label="Tática / Segurança" value={(delta?.tacticalIntelligence ?? 0).toFixed(1)} /><Metric label="Comp. / Liderança" value={(delta?.competitiveness ?? 0).toFixed(1)} /><Metric label="Momentum" value={(delta?.momentum ?? 0).toFixed(1)} />{delta?.historicalLearning!=null&&<Metric label="Histórico observado" value={Number(delta.historicalLearning).toFixed(2)} />}<Metric label="Pontuação" value={(delta?.score ?? 0).toFixed(2)} /></div></div>; }
