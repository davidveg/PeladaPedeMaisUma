...
function RoundRecapCard({recap,onShare}:any){return <section className="round-recap"><header><div><div className="eyebrow">RESENHA AUTOMÁTICA</div><h3>{recap.title}</h3><p>Gerada a partir do placar, da súmula, da votação e das conquistas desta partida.</p></div><span aria-hidden="true">📰</span></header><div className="round-recap-body"><div className="round-recap-score"><small>DESTAQUE DA RODADA</small><b>{recap.headline}</b></div><ul>{(recap.highlights||[]).map((highlight:string,index:number)=><li key={index}>{highlight}</li>)}</ul></div><div className="round-recap-actions"><button className="primary whatsapp-button" type="button" onClick={onShare}><WhatsAppIcon/>Compartilhar resenha</button></div></section>}
