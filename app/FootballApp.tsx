...
export default function FootballApp({ initialStage }: { initialStage?: InitialStage }) {
  ...
  async function confirmCareerMatch(separationId:string,blueScore:number,yellowScore:number,contributions:any[]=[],participation?:{blueIds:string[];yellowIds:string[]}){
    const response=await fetch('/api/career/match',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({separationId,blueScore,yellowScore,contributions,participationReviewed:true,participation})});
    const payload=await response.json().catch(()=>({}));
  ...
  async function editCareerResult(matchId:string,blueScore:number,yellowScore:number,contributions:any[]=[],participation?:{blueIds:string[];yellowIds:string[]}){
    const response=await fetch('/api/career/match',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({matchId,blueScore,yellowScore,contributions,participationReviewed:true,participation})});
    const payload=await response.json().catch(()=>({}));
...
export function SavedSeparation({ item, isAdmin, canCorrectConfirmedTeams = false, canManageResults = isAdmin, section = "all", careerConfig, publicBaseUrl, onConfirmCareer, onEditCareer, onSaveArrivalOrder, onSaveTeams, onBack, onShareLink, onPlayer, onCopy }: any) {
  ...
  const correctingConfirmed=Boolean(item.career&&canCorrectConfirmedTeams),canEditTeams=Boolean((isAdmin&&!item.career)||correctingConfirmed);
  const saveTeams=async()=>{const message=correctingConfirmed?'Confirmar a correção da escalação desta partida? O Momentum do resultado será recalculado. Os votos serão preservados, mas uma troca incompatível com gols ou assistências será bloqueada.':'Salvar esta nova distribuição? A ordem de chegada e um eventual rascunho de súmula serão limpos para evitar inconsistências.';if(!confirm(message))return;setTeamBusy(true);setTeamError('');try{await onSaveTeams(item.id,teamDraft.blue.map((player:Player)=>player.id),teamDraft.yellow.map((player:Player)=>player.id),correctingConfirmed);setEditingTeams(false)}catch(error:any){setTeamError(error.message)}finally{setTeamBusy(false)}};
  ...
  if (section === "result") return <>{item.career?.recap&&<RoundRecapCard recap={item.career.recap} onShare={onShareLink}/>}<CareerMatchCard item={item} isAdmin={canManageResults} section="result" enabled={careerConfig?.enabled!==false} trackContributions={careerConfig?.trackContributions!==false} publicBaseUrl={publicBaseUrl} onConfirm={onConfirmCareer} onEdit={onEditCareer}/></>;
  return <section className="content saved-detail"><div className="section-head"><div><div className="eyebrow">ESCALAÇÃO CONFIRMADA</div><h2>{item.matchTitle}</h2><p>{item.matchDate ? new Date(item.matchDate + "T12:00:00").toLocaleDateString("pt-BR") : "Data não informada"} · confirmada em {new Date(item.confirmedAt).toLocaleString("pt-BR")}{item.manuallyAdjusted ? " · ajustada manualmente" : ""}</p></div><BalanceBadge rating={result.rating || item.balanceClassification} /></div>{section==="all"&&item.career?.recap&&<RoundRecapCard recap={item.career.recap} onShare={onShareLink}/>} {section==="all"&&(item.career||canManageResults)&&<CareerMatchCard item={item} isAdmin={canManageResults} enabled={careerConfig?.enabled!==false} trackContributions={careerConfig?.trackContributions!==false} publicBaseUrl={publicBaseUrl} onConfirm={onConfirmCareer} onEdit={onEditCareer}/>} {canEditTeams&&<section className={`saved-team-editor ${editingTeams?'editing':''}`}><div><small>{correctingConfirmed?'CORREÇÃO DA ÚLTIMA PARTIDA':'AJUSTE ANTES DO RESULTADO'}</small><h3>Distribuição dos jogadores</h3><p>{editingTeams?'Use as setas nos jogadores para transferi-los ao outro time.':correctingConfirmed?'Corrija o lado em que cada jogador realmente atuou. Estatísticas e Momentum serão atualizados sem apagar a votação.':'Se alguém atrasar ou um time ficar desfalcado, ajuste os times antes de confirmar o resultado.'}</p></div>{editingTeams?<div className="saved-team-editor-actions"><button className="ghost" disabled={teamBusy} onClick={()=>{setTeamDraft(result);setEditingTeams(false);setTeamError('')}}>Cancelar</button><button className="primary" disabled={teamBusy} onClick={saveTeams}>{teamBusy?'Salvando…':correctingConfirmed?'Confirmar correção':'Salvar novos times'}</button></div>:<button className="ghost" onClick={()=>setEditingTeams(true)}>{correctingConfirmed?'Corrigir escalação':'Editar times'}</button>}{teamError&&<div className="alert error">{teamError}</div>}</section>} {!editingTeams&&<ArrivalOrder item={item} isAdmin={isAdmin} onSave={onSaveArrivalOrder}/>}<TeamGrid result={displayedResult} onPlayer={onPlayer} onMove={editingTeams?moveTeamPlayer:undefined}/><BalanceMetrics delta={displayedResult.delta} /><div className="saved-meta"><span><small>Proposta utilizada</small><b>{result.proposal || 1}</b></span><span><small>Peso da velocidade</small><b>{Math.round(weights.speedWeight * 100)}%</b></span><span><small>Peso da habilidade</small><b>{Math.round(weights.skillWeight * 100)}%</b></span><span><small>Peso da marcação</small><b>{Math.round(weights.markingWeight * 100)}%</b></span><span><small>Diferença máx. por posição</small><b>{result.maximumPositionDifference??"Não registrado"}</b></span><span><small>Melhores protegidos</small><b>{result.protectedTopPlayersPercentage==null?"Não registrado":`${Math.round(result.protectedTopPlayersPercentage*100)}%`}</b></span><span><small>Tentativas avaliadas</small><b>{result.algorithmAttempts ?? "Não registrado"}</b></span><span><small>Ajuste manual</small><b>{item.manuallyAdjusted ? "Sim" : "Não"}</b></span></div><div className="result-actions"><button className="ghost" onClick={onBack}>← Voltar ao histórico</button><button className="ghost" onClick={onShareLink}>{item.career?.recap?"Compartilhar resenha":"Compartilhar link da escalação"}</button><button className="ghost" onClick={() => onCopy(false)}>Copiar times</button><button className="primary" onClick={() => onCopy(true)}>Copiar com pontuações</button></div></section>;
}
...
function CareerMatchCard({item,isAdmin,section="all",enabled,trackContributions,publicBaseUrl,onConfirm,onEdit}:any){
 const {config:brand}=useInstanceBranding(),blueName=brand.teamBlueName,yellowName=brand.teamYellowName;
  ...
  const participation={blueIds:blueParticipantIds,yellowIds:yellowParticipantIds};
  const participationChanges=participationChangeSummary(bluePlayers,yellowPlayers,participation);
  async function submit(event:any){event.preventDefault();const detail=participationChanges.noShowNames.length?` ${participationChanges.noShowNames.length} escalado(s) marcado(s) como não participante(s): ${participationChanges.noShowNames.join(', ')}.`:'';if(!confirm(`Confirmar o placar ${blueScore} × ${yellowScore} e a participação efetiva de ${blueParticipantIds.length+yellowParticipantIds.length} jogadores?${detail} O momentum${trackContributions?' e as estatísticas de gols e assistências':''} será aplicado imediatamente.`))return;setBusy(true);setError('');try{await onConfirm(item.id,blueScore,yellowScore,contributions,participation);try{localStorage.removeItem(`ppm.match-draft.${item.id}`)}catch{}}catch(error:any){setError(error.message)}finally{setBusy(false)}}
  ...
  function beginEdit(){
    ...
    setBlueParticipantIds((career.participation?.blue||bluePlayers).map((player:any)=>String(player.id)));
    setYellowParticipantIds((career.participation?.yellow||yellowPlayers).map((player:any)=>String(player.id)));
    setError('');
    setEditingResult(true);
  }
  async function submitEdit(event:any){
    event.preventDefault();
    if(!confirm(`Salvar a correção do placar e da participação efetiva? O momentum e as estatísticas serão recalculados sem duplicar bônus ou ônus. Votos que envolvam jogadores removidos serão descartados.`))return;
    setBusy(true);setError('');
    try{await onEdit(career.id,blueScore,yellowScore,contributions,participation);setEditingResult(false)}catch(error:any){setError(error.message)}finally{setBusy(false)}
  }
  if(!career)return <section className={`career-match-card pending ${trackContributions?'with-contributions':''}`}><div><small>MODO CARREIRA</small><h3>Este jogo foi realizado?</h3><p>Confirme o placar e quem realmente participou para aplicar o momentum e abrir a votação dos destaques.{trackContributions?' Informe também o autor de cada gol e, quando houver, da assistência.':''}</p>{isAdmin&&enabled&&trackContributions&&<div className="match-draft-actions"><a className="ghost" href={`/sumula?separationId=${encodeURIComponent(item.id)}`}>Abrir rascunho da súmula ↗</a>{savedDraft?.contributions?.length>0&&<button className="ghost" type="button" onClick={applySavedDraft}>Usar rascunho ({savedDraft.blueScore} × {savedDraft.yellowScore})</button>}{savedDraft?.updatedAt&&<small>Atualizado em {new Date(savedDraft.updatedAt).toLocaleString('pt-BR')}</small>}</div>}</div>{isAdmin?(enabled?<form className="career-confirm-form" onSubmit={submit}><div className="career-score-inputs"><label>{`Time ${blueName}`}<input type="number" min="0" max="99" value={blueScore} onChange={e=>setBlueScore(Number(e.target.value))}/></label><b>×</b><label>{`Time ${yellowName}`}<input type="number" min="0" max="99" value={yellowScore} onChange={e=>setYellowScore(Number(e.target.value))}/></label></div><ParticipationEditor blueLineup={bluePlayers} yellowLineup={yellowPlayers} eligiblePlayers={eligiblePlayers} blueIds={blueParticipantIds} yellowIds={yellowParticipantIds} setBlueIds={setBlueParticipantIds} setYellowIds={setYellowParticipantIds}/>{trackContributions&&contributions.length>0&&<div className="contribution-editor"><div className="contribution-head"><b>Gols e assistências</b><small>Somente participantes efetivos podem receber gols ou assistências.</small></div>{contributions.map((goal,index)=>{const teamPlayers=goal.team==='BLUE'?actualBluePlayers:actualYellowPlayers,opponentPlayers=goal.team==='BLUE'?actualYellowPlayers:actualBluePlayers,scorerPlayers=goal.ownGoal?opponentPlayers:teamPlayers,teamNumber=contributions.slice(0,index+1).filter(item=>item.team===goal.team).length;return <div className={`contribution-row ${goal.team.toLowerCase()} ${goal.ownGoal?'own-goal':''}`} key={`${goal.team}-${teamNumber}`}><strong>{goal.team==='BLUE'?'🔵':'🟡'} Gol {teamNumber}{goal.ownGoal&&<em>GC</em>}</strong><label className="own-goal-check"><input type="checkbox" checked={Boolean(goal.ownGoal)} onChange={event=>setContribution(index,'ownGoal',event.target.checked)}/> GC (Gol Contra)</label><label>{goal.ownGoal?'Jogador que marcou contra':'Autor'}<select value={goal.scorerPlayerId} onChange={event=>setContribution(index,'scorerPlayerId',event.target.value)} required><option value="">Selecione</option>{scorerPlayers.map((player:Player)=><option value={player.id} key={player.id}>{player.displayName}</option>)}</select></label><label>Assistência<select value={goal.assistPlayerId} onChange={event=>setContribution(index,'assistPlayerId',event.target.value)} disabled={goal.ownGoal}><option value="">{goal.ownGoal?'Não se aplica':'Sem assistência'}</option>{!goal.ownGoal&&teamPlayers.filter((player:Player)=>player.id!==goal.scorerPlayerId).map((player:Player)=><option value={player.id} key={player.id}>{player.displayName}</option>)}</select></label></div>})}</div>}<span className="draft-autosave-status" role="status">{draftStatus}</span><button className="primary" disabled={busy}>{busy?'Confirmando…':'Confirmar partida'}</button>{error&&<span className="career-error">{error}</span>}</form>:<div className="alert">O Modo Carreira está desativado nas configurações administrativas.</div>):null}</section>;
  if(editingResult)return <CareerResultEditor blueScore={blueScore} yellowScore={yellowScore} setBlueScore={setBlueScore} setYellowScore={setYellowScore} contributions={contributions} setContribution={setContribution} bluePlayers={actualBluePlayers} yellowPlayers={actualYellowPlayers} blueLineup={bluePlayers} yellowLineup={yellowPlayers} eligiblePlayers={eligiblePlayers} blueIds={blueParticipantIds} yellowIds={yellowParticipantIds} setBlueIds={setBlueParticipantIds} setYellowIds={setYellowParticipantIds} trackContributions={trackContributions} busy={busy} error={error} onSubmit={submitEdit} onCancel={()=>{setEditingResult(false);setError('')}}/>;
  const winner=career.winnerTeam==='BLUE'?`Time ${blueName}`:career.winnerTeam==='YELLOW'?`Time ${yellowName}`:'Empate',rules=career.config;
  return <section className="career-match-card confirmed"><div className="career-score">{isAdmin&&<button className="ghost career-edit-result" onClick={beginEdit}>Editar resultado</button>}<small>PLACAR CONFIRMADO</small><strong><span className="blue">{blueName} <b>{career.blueScore}</b></span><i>×</i><span className="yellow"><b>{career.yellowScore}</b> {yellowName}</span></strong><em>{winner}</em>{rules&&<p className="career-rules">Regras do Momentum registradas nesta partida</p>}</div><ParticipationSummary blueLineup={bluePlayers} yellowLineup={yellowPlayers} participation={career.participation}/>{rules&&<CareerRulesSnapshot rules={rules}/>} {trackContributions&&career.contributions?.length>0&&<CareerContributions contributions={career.contributions}/>}{section!=="result"&&<><div className="career-voting-share"><div><small>{career.status==='OPEN'?'VOTAÇÃO ABERTA':'VOTAÇÃO ENCERRADA'}</small><h3>Destaques da partida</h3><p>{career.status==='OPEN'?`Votos aceitos até ${new Date(career.closesAt).toLocaleString('pt-BR')}.`:'Resultado final consolidado; os votos não podem mais ser alterados.'}</p>{career.status==='OPEN'&&<div className="career-link-actions"><button className="ghost" onClick={copy}>Copiar link</button><button className="primary whatsapp-button" onClick={whatsapp}><WhatsAppIcon/>Compartilhar no WhatsApp</button></div>}</div>{career.status==='OPEN'&&qr&&<a href={votingUrl} target="_blank" rel="noreferrer"><img src={qr} alt="QR Code para votação da partida"/><span>Abrir votação</span></a>}</div>{career.status==='CLOSED'&&<CareerPublicResults results={career.results} players={[...(career.participation?.blue||item.snapshot.blue),...(career.participation?.yellow||item.snapshot.yellow)]} onShare={enabled?shareVotingResults:null}/>}</>}</section>
}
function CareerResultEditor({blueScore,yellowScore,setBlueScore,setYellowScore,contributions,setContribution,bluePlayers,yellowPlayers,blueLineup,yellowLineup,eligiblePlayers,blueIds,yellowIds,setBlueIds,setYellowIds,trackContributions,busy,error,onSubmit,onCancel}:any){
  const {config:brand}=useInstanceBranding(),blueName=brand.teamBlueName,yellowName=brand.teamYellowName;
  return <section className="career-match-card career-result-editor">
    <div className="career-result-editor-head">
      <div><small>CORREÇÃO ADMINISTRATIVA</small><h3>Editar resultado da partida</h3><p>Corrija o placar, a participação efetiva e, quando habilitado, os autores dos gols e assistências. Votos que envolvam alguém removido serão descartados.</p></div>
    </div>
    <form className="career-confirm-form" onSubmit={onSubmit}>
      ...
      <ParticipationEditor blueLineup={blueLineup} yellowLineup={yellowLineup} eligiblePlayers={eligiblePlayers} blueIds={blueIds} yellowIds={yellowIds} setBlueIds={setBlueIds} setYellowIds={setYellowIds}/>
...
function ParticipationEditor({blueLineup,yellowLineup,eligiblePlayers,blueIds,yellowIds,setBlueIds,setYellowIds}:any){
  const {config:brand}=useInstanceBranding(),selected=new Set([...blueIds,...yellowIds]),byId=Object.fromEntries(eligiblePlayers.map((player:any)=>[String(player.id),player])),lineupIds=new Set([...blueLineup,...yellowLineup].map((player:any)=>String(player.id))),available=eligiblePlayers.filter((player:any)=>!selected.has(String(player.id)));
  const toggle=(id:string,team:"BLUE"|"YELLOW",played:boolean)=>{const own=team==="BLUE"?blueIds:yellowIds,other=team==="BLUE"?yellowIds:blueIds,setOwn=team==="BLUE"?setBlueIds:setYellowIds,setOther=team==="BLUE"?setYellowIds:setBlueIds;setOther(other.filter((value:string)=>value!==id));setOwn(played?[...own.filter((value:string)=>value!==id),id]:own.filter((value:string)=>value!==id))};
  const team=(label:string,color:string,lineup:any[],ids:string[],side:"BLUE"|"YELLOW")=>{const manual=ids.filter(id=>!lineupIds.has(id));return <section className={`participation-team ${color}`}><h5>Time {label}<span>{ids.length} jogaram</span></h5><div>{lineup.map((player:any)=><label key={player.id} className={ids.includes(String(player.id))?'played':'no-show'}><input type="checkbox" checked={ids.includes(String(player.id))} onChange={event=>toggle(String(player.id),side,event.target.checked)}/><span><b>{player.displayName}</b><small>{ids.includes(String(player.id))?'Jogou':'Não jogou'}</small></span></label>)}{manual.map(id=>{const player=byId[id];return player?<div className="manual-participant" key={id}><span><b>{player.displayName}</b><small>Adicionado à participação</small></span><button type="button" onClick={()=>toggle(id,side,false)} aria-label={`Remover ${player.displayName}`}>×</button></div>:null})}</div><label className="participant-add"><span>Adicionar quem jogou de última hora</span><select value="" onChange={event=>{if(event.target.value)toggle(event.target.value,side,true)}}><option value="">Selecione um jogador</option>{available.map((player:any)=><option key={player.id} value={player.id}>{player.displayName}</option>)}</select></label></section>};
  return <fieldset className="participation-editor"><legend>Participação efetiva</legend><p>Confirme quem realmente entrou em campo. Somente esses jogadores recebem resultado, estatísticas e acesso à votação.</p><div>{team(brand.teamBlueName,"blue",blueLineup,blueIds,"BLUE")}{team(brand.teamYellowName,"yellow",yellowLineup,yellowIds,"YELLOW")}</div></fieldset>
}
function participationChangeSummary(blueLineup:any[],yellowLineup:any[],participation:{blueIds:string[];yellowIds:string[]}){const actual=new Set([...participation.blueIds,...participation.yellowIds]),lineup=[...blueLineup,...yellowLineup];return{noShowNames:lineup.filter(player=>!actual.has(String(player.id))).map(player=>player.displayName),addedCount:[...actual].filter(id=>!lineup.some(player=>String(player.id)===id)).length}}
function ParticipationSummary({blueLineup,yellowLineup,participation}:any){if(!participation)return <div className="participation-summary legacy"><b>Participação presumida pela escalação</b><span>Este é um resultado anterior ao registro de participação efetiva.</span></div>;const summary=participationChangeSummary(blueLineup,yellowLineup,{blueIds:(participation.blue||[]).map((player:any)=>String(player.id)),yellowIds:(participation.yellow||[]).map((player:any)=>String(player.id))}),played=(participation.blue||[]).length+(participation.yellow||[]).length;return <div className={`participation-summary ${summary.noShowNames.length?'has-no-shows':''}`}><b>{played} participantes efetivos</b><span>{summary.noShowNames.length?`${summary.noShowNames.join(', ')} não participou${summary.noShowNames.length===1?'':'aram'} e não recebeu resultado ou estatísticas.`:'Todos os jogadores escalados participaram.'}{summary.addedCount?` ${summary.addedCount} jogador${summary.addedCount===1?' foi adicionado':'es foram adicionados'} no fechamento.`:''}</span></div>}
function signed(value:number,digits=1){return `${value>0?'+':''}${Number(value).toLocaleString('pt-BR',{minimumFractionDigits:digits,maximumFractionDigits:digits})}`}
function CareerRulesSnapshot({rules}:any){const value=(key:string,fallback:number)=>Number.isFinite(Number(rules?.[key]))?Number(rules[key]):fallback,decimal=(number:number,digits=1)=>number.toLocaleString('pt-BR',{minimumFractionDigits:digits,maximumFractionDigits:digits}),percent=(number:number)=>`${decimal(number*100,number*100%1?1:0)}%`,resultMultiplier=value('resultMomentumMultiplier',1),votingMultiplier=value('momentumMultiplier',1),winnerBonus=value('winnerBonus',.1),motmFirst=value('motmFirst',.3);return <section className="career-rules-snapshot"><header><div><small>CONFIGURAÇÃO HISTÓRICA</small><h3>Como o Momentum foi configurado</h3></div><p>Os pontos abaixo formam saldos de Momentum separados. Cada multiplicador atua somente sobre seu saldo; a contribuição calculada é então somada ao OVR-base.</p></header><div className="career-rule-weights"><article><small>MOMENTUM DE RESULTADOS</small><b>{decimal(resultMultiplier)}×</b><span>{percent(resultMultiplier)} do saldo de vitórias e derrotas</span></article><article><small>MOMENTUM DE VOTAÇÕES</small><b>{decimal(votingMultiplier)}×</b><span>{percent(votingMultiplier)} do saldo obtido nas votações</span></article></div><p className="career-rule-example"><b>Exemplo:</b> vitória {signed(winnerBonus)} × {decimal(resultMultiplier)} = {signed(winnerBonus*resultMultiplier,2)} no OVR; 1º no Man of the Match {signed(motmFirst)} × {decimal(votingMultiplier)} = {signed(motmFirst*votingMultiplier,2)} no OVR.</p><small className="career-rule-points-label">PONTOS BRUTOS DE MOMENTUM</small><div className="career-rule-points"><article><b>Resultado da partida</b><span>Vitória <strong>{signed(winnerBonus)}</strong></span><span>Empate <strong>0,0</strong></span><span>Derrota <strong>{signed(value('loserPenalty',-.1))}</strong></span></article><article><b>Man of the Match</b><span>1º <strong>{signed(motmFirst)}</strong></span><span>2º <strong>{signed(value('motmSecond',.2))}</strong></span><span>3º <strong>{signed(value('motmThird',.1))}</strong></span></article><article><b>Deception of the Match</b><span>1º <strong>{signed(value('dotmFirst',-.3))}</strong></span><span>2º <strong>{signed(value('dotmSecond',-.2))}</strong></span><span>3º <strong>{signed(value('dotmThird',-.1))}</strong></span></article><article><b>Reconhecimentos</b><span>Parceiro <strong>{signed(value('partnerAward',.1))}</strong></span><span>Fair Play <strong>{signed(value('fairPlayAward',.1))}</strong></span><span>Defesa <strong>{signed(value('defenseAward',.1))}</strong></span></article></div></section>}
...
function BalanceMetrics({ delta, extraId }: any) {
  ...
  const hasMidfieldProfiles=delta?.defensiveMidfielders!=null||delta?.centralMidfielders!=null||delta?.offensiveMidfielders!=null;
  return <div className="metrics"><h3>Diferenças entre os times</h3>{usesBaseTeams&&<p className="base-team-note">Os atributos e a pontuação comparam times-base do mesmo tamanho. O jogador adicional aparece separado.</p>}<div>
    <Metric label="Jogadores" value={delta?.players ?? 0} side={side('players',delta?.players)} blueName={blueName} yellowName={yellowName}/>
    <Metric label="Defensores" value={delta?.defenders ?? 0} side={side('defenders',delta?.defenders)} blueName={blueName} yellowName={yellowName}/>
    <Metric label="Meio-campistas" value={delta?.midfielders ?? 0} side={side('midfielders',delta?.midfielders)} blueName={blueName} yellowName={yellowName}/>
    <Metric label="Atacantes" value={delta?.attackers ?? 0} side={side('attackers',delta?.attackers)} blueName={blueName} yellowName={yellowName}/>
    {hasMidfieldProfiles&&<><Metric label="Meias defensivos" value={delta?.defensiveMidfielders ?? 0} side={side('defensiveMidfielders',delta?.defensiveMidfielders)} blueName={blueName} yellowName={yellowName} help="Meio-campistas com Defesa como posição secundária. O algoritmo evita concentrar esse perfil em apenas um time."/><Metric label="Meias centrais" value={delta?.centralMidfielders ?? 0} side={side('centralMidfielders',delta?.centralMidfielders)} blueName={blueName} yellowName={yellowName}/><Metric label="Meias ofensivos" value={delta?.offensiveMidfielders ?? 0} side={side('offensiveMidfielders',delta?.offensiveMidfielders)} blueName={blueName} yellowName={yellowName}/></>}
    <Metric label="Físico / Pos." value={(delta?.speed ?? 0).toFixed(1)} side={side('speed',(delta?.speed??0).toFixed(1))} blueName={blueName} yellowName={yellowName}/><Metric label="Técnica / Def." value={(delta?.skill ?? 0).toFixed(1)} side={side('skill',(delta?.skill??0).toFixed(1))} blueName={blueName} yellowName={yellowName}/><Metric label="Marcação / Pés" value={(delta?.marking ?? 0).toFixed(1)} side={side('marking',(delta?.marking??0).toFixed(1))} blueName={blueName} yellowName={yellowName}/><Metric label="Tática / Segurança" value={(delta?.tacticalIntelligence ?? 0).toFixed(1)} side={side('tacticalIntelligence',(delta?.tacticalIntelligence??0).toFixed(1))} blueName={blueName} yellowName={yellowName}/><Metric label="Comp. / Liderança" value={(delta?.competitiveness ?? 0).toFixed(1)} side={side('competitiveness',(delta?.competitiveness??0).toFixed(1))} blueName={blueName} yellowName={yellowName}/><Metric label="Momentum" value={(delta?.momentum ?? 0).toFixed(1)} side={side('momentum',(delta?.momentum??0).toFixed(1))} blueName={blueName} yellowName={yellowName} help="Diferença entre os saldos de momentum de resultados e votações usados no overall dos jogadores."/>{delta?.historicalLearning!=null&&<Metric label="Histórico observado" value={Number(delta.historicalLearning).toFixed(2)} side={side('historicalLearning',Number(delta.historicalLearning).toFixed(2))} blueName={blueName} yellowName={yellowName} help="Diferença do ajuste histórico interno que considera resultados, saldo de gols, gols, assistências, avaliações e desempenho recente. Esse índice não altera o OVR exibido."/>}<Metric label="Pontuação" value={(delta?.score ?? 0).toFixed(2)} side={side('score',(delta?.score??0).toFixed(2))} blueName={blueName} yellowName={yellowName} help={usesBaseTeams?"Diferença entre as somas dos overalls dos dois times-base, sem o jogador adicional.":"Diferença entre as somas dos overalls dos dois times. Quanto mais próximo de zero, mais equilibradas estão as pontuações."}/>
  </div></div>;
}
