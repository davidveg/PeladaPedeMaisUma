...
function CareerMatchCard({item,isAdmin,section="all",enabled,trackContributions,publicBaseUrl,onConfirm,onEdit}:any){
 const {config:brand}=useInstanceBranding(),blueName=brand.teamBlueName,yellowName=brand.teamYellowName;
  ...
  useEffect(()=>{if(!isAdmin||!enabled)return;fetch(`/api/career/draft?separationId=${encodeURIComponent(item.id)}`,{cache:'no-store'}).then(async response=>{const payload=await response.json();if(response.ok){setSavedDraft(payload.draft);setEligiblePlayers(payload.eligiblePlayers||[...bluePlayers,...yellowPlayers]);if(payload.participation?.reviewed){setBlueParticipantIds(payload.participation.blueIds||[]);setYellowParticipantIds(payload.participation.yellowIds||[])}}}).catch(()=>undefined)},[item.id,isAdmin,enabled]);
