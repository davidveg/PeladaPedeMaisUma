...
export default function FootballApp() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {stage === "history" && historyDetail && <SavedSeparation item={historyDetail} isAdmin={isAdmin} careerConfig={careerConfig} publicBaseUrl={publicBaseUrl} onConfirmCareer={confirmCareerMatch} onEditCareer={editCareerResult} onSaveArrivalOrder={saveArrivalOrder} onBack={() => setHistoryDetail(null)} onPlayer={(player:Player)=>showPlayer(player,{...resultConfig(historyDetail.snapshot),showContributions:publicPlayerConfig.showContributions})} onCopy={(withScores: boolean) => copyTeams(historyDetail.snapshot, withScores, historyDetail.matchTitle)} />}
    </main>
...
function SavedSeparation({ item, isAdmin, careerConfig, publicBaseUrl, onConfirmCareer, onEditCareer, onSaveArrivalOrder, onBack, onPlayer, onCopy }: any) {
...
function CareerMatchCard({item,isAdmin,enabled,trackContributions,publicBaseUrl,onConfirm,onEdit}:any){
  ...
  const [blueScore,setBlueScore]=useState(0),[yellowScore,setYellowScore]=useState(0),[contributions,setContributions]=useState<any[]>([]),[busy,setBusy]=useState(false),[error,setError]=useState(''),[qr,setQr]=useState(''),[editingResult,setEditingResult]=useState(false);
