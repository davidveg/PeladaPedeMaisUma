...
export default function FootballApp({ initialStage }: { initialStage?: InitialStage }) {
  ...
  async function shareSavedSeparation(item:any){const url=savedSeparationUrl(item),text=item.career?.recap?.shareText||`⚽ ${item.matchTitle}\nConfira os times e os detalhes desta escalação:`;if(navigator.share){try{await navigator.share({title:item.career?.recap?.title||item.matchTitle,text,url});return}catch(error:any){if(error?.name==="AbortError")return}}await navigator.clipboard.writeText(`${text}\n\n${url}`);setToast(item.career?.recap?"Resenha copiada.":"Link da escalação copiado.")}
