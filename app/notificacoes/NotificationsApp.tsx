...
export default function NotificationsApp() {
  ...
  async function read(item?: Notice) {
    ...
    if (item?.matchId) window.location.assign(`/partidas#${encodeURIComponent(item.matchId)}`);
    else await load();
  }
