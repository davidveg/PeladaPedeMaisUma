...
function MemberProfile({ player, config, onEdit }: { player: Player; config: Config; onEdit: () => void }) {
  ...
  return <div className="member-profile-layout"><section className={`member-card tier-${tier}`}><div className="member-card-top"><div className="member-overall"><strong>{overall.toFixed(1)}</strong><span className="member-overall-label">OVERALL</span>{config.cardTiersEnabled&&<em className="member-tier-badge">{playerCardTierLabel(tier)}</em>}</div><PlayerPhoto photoUrl={player.photoUrl} name={player.displayName} large /></div><h2>{player.displayName}</h2><div className="member-role"><span><small>TIPO</small><b>{playerTypeLabel(player.type)}</b></span><span><small>POSIÇÃO</small><b>{player.primaryPosition}</b></span></div><div className="card-stats">{stats.map((stat,index)=><span key={stat.label}><b>{Number(stat.value).toFixed(1)}</b><small className="card-stat-label"><span>{stat.label}</span><MemberCardDisciplineHelp id={`member-card-discipline-${player.id}-${index}`} label={stat.label} text={stat.help}/></small></span>)}<span><b>{(player.momentum??0)>0?"+":""}{Number(player.momentum??0).toFixed(1)}</b><small>MOMENTUM</small></span></div><div className={`card-career-stats ${config.showContributions?'with-contributions':''}`}><span><b>{career.games}</b><small>JOGOS</small></span><span className="wins"><b>{career.wins}</b><small>VITÓRIAS</small></span><span className="losses"><b>{career.losses}</b><small>DERROTAS</small></span>{config.showContributions&&<><span className="goals"><b>{career.goals??0}</b><small>GOLS</small></span><span className="assists"><b>{career.assists??0}</b><small>ASSISTÊNCIAS</small></span></>}</div></section><section className="member-profile-actions"><div><div className="eyebrow">MEU PERFIL</div><h2>Informações do jogador</h2><p>Você pode manter seus dados pessoais atualizados. Notas esportivas e momentum continuam sob responsabilidade dos organizadores.</p></div><dl><div><dt>Nome completo</dt><dd>{player.fullName}</dd></div><div><dt>Apelido</dt><dd>{player.nickname || "Não informado"}</dd></div><div><dt>Posição</dt><dd>{player.primaryPosition}</dd></div><div><dt>Observações</dt><dd>{player.notes || "Nenhuma observação"}</dd></div></dl><button className="primary" onClick={onEdit}>Editar minhas informações</button></section></div>;
}
