...
function MemberProfile({ player, config, onEdit }: { player: Player; config: Config; onEdit: () => void }) {
  ...
  return <div className="member-profile-layout"><section className={`member-card tier-${tier}`}><div className="member-card-top"><div className="member-overall"><strong>{overall.toFixed(1)}</strong><span>{config.cardTiersEnabled?`OVERALL · ${playerCardTierLabel(tier)}`:'OVERALL'}</span></div><PlayerPhoto photoUrl={player.photoUrl} name={player.displayName} large /></div><h2>{player.displayName}</h2><div className="member-role"><span><small>TIPO</small><b>{player.type === "guest" ? "Convidado" : goalkeeper ? "Goleiro" : "Mensalista"}</b></span><span><small>POSIÇÃO</small><b>{player.primaryPosition}</b></span></div><div className="member-attributes">{goalkeeper ? <><Attribute label="POSICIONAMENTO" value={player.goalkeeperPositioning ?? player.speed} /><Attribute label="HABILIDADE" value={player.skill} /><Attribute label="SAÍDA DE GOL" value={player.goalExit ?? player.marking ?? 3} /></> : <><Attribute label="VELOCIDADE" value={player.speed} /><Attribute label="HABILIDADE" value={player.skill} /><Attribute label="MARCAÇÃO" value={player.marking ?? 3} /></>}<Attribute label="MOMENTUM" value={player.momentum ?? 0} signed /></div><div className={`card-career-stats ${config.showContributions?'with-contributions':''}`}><span><b>{career.games}</b><small>JOGOS</small></span><span className="wins"><b>{career.wins}</b><small>VITÓRIAS</small></span><span className="losses"><b>{career.losses}</b><small>DERROTAS</small></span>{config.showContributions&&<><span className="goals"><b>{career.goals??0}</b><small>GOLS</small></span><span className="assists"><b>{career.assists??0}</b><small>ASSISTÊNCIAS</small></span></>}</div></section><section className="member-profile-actions"><div><div className="eyebrow">MEU PERFIL</div><h2>Informações do jogador</h2><p>Você pode manter seus dados pessoais atualizados. Notas esportivas e momentum continuam sob responsabilidade dos organizadores.</p></div><dl><div><dt>Nome completo</dt><dd>{player.fullName}</dd></div><div><dt>Apelido</dt><dd>{player.nickname || "Não informado"}</dd></div><div><dt>Posição</dt><dd>{player.primaryPosition}</dd></div><div><dt>Observações</dt><dd>{player.notes || "Nenhuma observação"}</dd></div></dl><button className="primary" onClick={onEdit}>Editar minhas informações</button></section></div>;
}
