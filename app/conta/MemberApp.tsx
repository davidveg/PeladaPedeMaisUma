...
import { uploadBrowserImage } from "../../lib/browser-image-upload";
...
function PlayerEngagementPanels({ engagement }: { engagement: PlayerEngagement }) {
  ...
  return <div className="player-engagement-layout">
    <section className="player-achievements"><div className="engagement-panel-head"><div><div className="eyebrow">CONQUISTAS</div><h2>Minha coleção</h2><p>Marcos liberados automaticamente pelos resultados oficiais da pelada.</p></div><strong>{engagement.achievements.unlocked.length}<small>desbloqueadas</small></strong></div>
      {engagement.achievements.unlocked.length ? <div className="achievement-grid">{engagement.achievements.unlocked.map(item=><article key={item.id}><i aria-hidden="true">{item.icon}</i><div><b>{item.title}</b><p>{item.description}</p><small>{new Date(`${item.achievedAt.slice(0,10)}T12:00:00`).toLocaleDateString("pt-BR")}</small>{item.matchId&&<a href={`/separacoes-salvas?separation=${encodeURIComponent(item.matchId)}`}>Ver partida →</a>}</div></article>)}</div> : <div className="engagement-empty">A primeira conquista aparecerá após um resultado oficial.</div>}
    ...
    <section className="season-retrospective"><div className="engagement-panel-head"><div><div className="eyebrow">RETROSPECTIVA PESSOAL</div><h2>{season.title}</h2><p>{season.summary}</p></div><button className="primary" type="button" onClick={share}>{copied?"Resumo copiado ✓":"Compartilhar resumo"}</button></div>
      ...
      <div className="retrospective-stories"><article><i>🔥</i><span><small>MELHOR SEQUÊNCIA</small><b>{season.bestWinningStreak} {season.bestWinningStreak===1?"vitória":"vitórias"}</b></span></article><article><i>🤝</i><span><small>PARCERIA MAIS FREQUENTE</small><b>{season.topPartner?`${season.topPartner.displayName} · ${season.topPartner.games} jogos`:"Ainda sem dados"}</b></span></article><article><i>⭐</i><span><small>RECONHECIMENTOS</small><b>{season.motmAwards}× craque · {season.partnerAwards}× parceiro · {season.fairPlayAwards}× Fair Play · {season.defenseAwards}× defesa</b></span></article><article><i>🏅</i><span><small>SELEÇÕES DO MÊS</small><b>{season.monthlySelections} · {season.playerOfMonthAwards}× jogador do mês</b></span></article></div>
    </section>
  </div>;
}
...
function MemberProfile({ player, config, onEdit }: { player: Player; config: Config; onEdit: () => void }) {
  ...
  return <div className="member-profile-layout"><section className={`member-card tier-${tier}`}><div className="member-card-top"><div className="member-overall"><strong>{overall.toFixed(1)}</strong><span className="member-overall-label">OVERALL</span>{config.cardTiersEnabled&&<em className="member-tier-badge">{playerCardTierLabel(tier)}</em>}</div><PlayerPhoto photoUrl={player.photoUrl} name={player.displayName} large /></div><h2>{player.displayName}</h2><div className={`member-role${player.secondaryPosition?' has-secondary':''}`}><span><small>TIPO</small><b>{playerTypeLabel(player.type)}</b></span><span><small>POSIÇÃO PRINCIPAL</small><b>{player.primaryPosition}</b></span>{player.secondaryPosition&&<span><small>POSIÇÃO SECUNDÁRIA</small><b>{player.secondaryPosition}</b></span>}</div><div className="card-stats">{stats.map((stat,index)=><span key={stat.label}><b>{Number(stat.value).toFixed(1)}</b><small className="card-stat-label"><span>{stat.label}</span><MemberCardDisciplineHelp id={`member-card-discipline-${player.id}-${index}`} label={stat.label} text={stat.help}/></small></span>)}<span><b>{(player.momentum??0)>0?"+":""}{Number(player.momentum??0).toFixed(1)}</b><small>MOMENTUM</small></span></div><div className={`card-career-stats ${config.showContributions?'with-contributions':''}`}><span><b>{career.games}</b><small>JOGOS</small></span><span className="wins"><b>{career.wins}</b><small>VITÓRIAS</small></span><span className="losses"><b>{career.losses}</b><small>DERROTAS</small></span>{config.showContributions&&<><span className="goals"><b>{career.goals??0}</b><small>GOLS</small></span><span className="assists"><b>{career.assists??0}</b><small>ASSISTÊNCIAS</small></span></>}</div></section><section className="member-profile-actions"><div><div className="eyebrow">MEU PERFIL</div><h2>Informações do jogador</h2><p>Você pode manter seus dados pessoais atualizados. Notas esportivas e momentum continuam sob responsabilidade dos organizadores.</p></div><dl><div><dt>Nome completo</dt><dd>{player.fullName}</dd></div><div><dt>Apelido</dt><dd>{player.nickname || "Não informado"}</dd></div><div><dt>Posição principal</dt><dd>{player.primaryPosition}</dd></div>{player.secondaryPosition&&<div><dt>Posição secundária</dt><dd>{player.secondaryPosition}</dd></div>}<div><dt>Observações</dt><dd>{player.notes || "Nenhuma observação"}</dd></div></dl><button className="primary" onClick={onEdit}>Editar minhas informações</button><a className="member-advanced-statistics-link" href={`/estatisticas/avancadas?player=${encodeURIComponent(player.id)}`}>Ver análise completa →</a></section></div>;
}
...
function MemberProfileForm({ player, onClose, onSaved }: { player: Player; onClose: () => void; onSaved: (message: string) => Promise<void> }) {
  const [form, setForm] = useState({ fullName: player.fullName, nickname: player.nickname || "", primaryPosition: player.primaryPosition, secondaryPosition: player.secondaryPosition || "", notes: player.notes || "", photoUrl: player.photoUrl || null }), [uploading, setUploading] = useState(false), [saving, setSaving] = useState(false), [error, setError] = useState("");
  const set = (key: string, value: any) => setForm(current => ({ ...current, [key]: value }));
  const setPrimaryPosition = (primaryPosition: string) => setForm(current => ({ ...current, primaryPosition: primaryPosition as Player["primaryPosition"], secondaryPosition: primaryPosition === "Goleiro" || current.secondaryPosition === primaryPosition ? "" : current.secondaryPosition }));
  async function upload(file?: File) { if (!file) return; setUploading(true); setError(""); try { set("photoUrl", await uploadBrowserImage(file, { variant: "photo" })); } catch (cause: any) { setError(cause.message); } finally { setUploading(false); } }
