...
type Payload = {
  ...
  versus: { playerA: Player | null; playerB: Player | null; winsA: number; winsB: number; draws: number; matches: VersusMatch[]; totalMatches?: number; matchDetailsRestricted?: boolean };
};
...
export default function StatisticsApp() {
  ...
  return <div className="statistics-page"><SiteHeader active="statistics"/><main className="statistics-main">
    ...
    <div className="statistics-advanced-entry"><a className="statistics-advanced-link" href="/estatisticas/avancadas">Abrir estatísticas avançadas →</a></div>
    ...
    {loading&&!data?<div className="statistics-loading">Calculando estatísticas…</div>:data&&<>
      ...
      {versus?.playerA&&versus.playerB?<><div className="versus-score"><PlayerSide player={versus.playerA} wins={versus.winsA}/><div><small>{versus.totalMatches ?? versus.matches.length} confrontos</small><b>{versus.winsA} <i>×</i> {versus.winsB}</b><span>{versus.draws} empates</span></div><PlayerSide player={versus.playerB} wins={versus.winsB}/></div><div className="versus-matches">{versus.matchDetailsRestricted ? <div className="empty">Entre na sua conta para consultar os detalhes das partidas deste confronto. <a href={`/conta?returnTo=${encodeURIComponent(typeof window === "undefined" ? "/estatisticas" : window.location.pathname + window.location.search)}`}>Entrar na minha conta</a></div> : versus.matches.length?versus.matches.map(match=><a href={`/separacoes-salvas?separation=${encodeURIComponent(match.separationId)}`} key={match.id}><span><b>{new Date(match.date+"T12:00:00").toLocaleDateString("pt-BR")}</b><small>{match.title}</small></span><strong className={match.result==="A"?"winner":""}>{match.teamA==="BLUE"?match.blueScore:match.yellowScore}</strong><i>×</i><strong className={match.result==="B"?"winner":""}>{match.teamB==="BLUE"?match.blueScore:match.yellowScore}</strong><em>{match.result==="DRAW"?"Empate":match.result==="A"?`${versus.playerA!.displayName} venceu`:`${versus.playerB!.displayName} venceu`}</em></a>):<div className="empty">Nenhum confronto em equipes adversárias no período.</div>}</div></>:<div className="versus-empty">Selecione dois jogadores para comparar o retrospecto.</div>}</section>
    </>}</main></div>;
}
