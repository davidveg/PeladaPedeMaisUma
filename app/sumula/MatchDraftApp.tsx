...
export default function MatchDraftApp() {
  ...
  return <main className="match-draft-page">
    ...
    <section className="match-draft-hero"><div><small>RASCUNHO DA SÚMULA</small><h1>{data.matchTitle}</h1><p>{data.matchDate?new Date(data.matchDate+"T12:00:00").toLocaleDateString("pt-BR"):"Data não informada"}</p></div><div className="match-draft-score"><span className="blue">{brand.teamBlueName} <b>{scores.blue}</b></span><i>×</i><span className="yellow"><b>{scores.yellow}</b> {brand.teamYellowName}</span></div></section>
    ...
    {!data.officialResultConfirmed&&data.enabled&&data.trackContributions&&<>
      <section className="match-draft-add"><button className="draft-blue" onClick={()=>addGoal("BLUE")}>{`+ Gol do Time ${brand.teamBlueName}`}</button><button className="draft-yellow" onClick={()=>addGoal("YELLOW")}>{`+ Gol do Time ${brand.teamYellowName}`}</button></section>
