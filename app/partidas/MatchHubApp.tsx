...
export default function MatchHubApp() {
  ...
  return <div className="member-page"><SiteHeader active="matches"/><main className="match-hub-main">
    ...
    {accessRequired ? <SignIn returnTo={returnTo}/> : loading || search === null ? <div className="empty" role="status">Carregando partidas…</div> : !data ? null : detail ? item ? <>
      ...
      {!item.matchId && <p className="match-hub-help">Escalação do histórico, sem partida agendada vinculada. Não há lista de presenças registrada.</p>}
...
function RoundCenter({item,onNavigate}:{item:MatchHubItem;onNavigate:(tab:string)=>void}){
  const attendance:Record<string,string>={PRESENT:"Presença confirmada",ABSENT:"Ausência informada",WAITLIST:"Na lista de espera"};
  const vote:Record<string,string>={AVAILABLE:"Voto pendente",DONE:"Voto registrado",NOT_PARTICIPANT:"Você não participou",CLOSED:"Votação encerrada"};
  return <section className="round-center"><header><div><small>CENTRAL DESTA RODADA</small><h3>{item.status==="FINISHED"?"Resultado e próximos passos":"Tudo o que importa agora"}</h3></div>{item.nextAction&&<button className="primary" type="button" onClick={()=>onNavigate(item.nextAction!.tab)}>{item.nextAction.label} →</button>}</header><div className="round-center-grid"><article><span>📅</span><div><small>AGENDA</small><b>{dateLabel(item.date)}</b><p>{item.confirmationDeadline?`Confirme até ${dateLabel(item.confirmationDeadline)}`:"Prazo não informado"}</p></div></article><article><span>👟</span><div><small>MINHA SITUAÇÃO</small><b>{item.viewerAttendanceStatus?attendance[item.viewerAttendanceStatus]||item.viewerAttendanceStatus:item.status==="OPEN"?"Resposta pendente":"Rodada encerrada"}</b><p>{item.present!==null?`${item.present} presença${item.present===1?"":"s"} confirmada${item.present===1?"":"s"}`:"Histórico sem lista de presença"}</p></div></article><article><span>📍</span><div><small>LOCAL E CLIMA</small><b>{item.location||"Local não informado"}</b><p>{weatherCenterLabel(item)}</p></div></article><article><span>🏆</span><div><small>PÓS-JOGO</small><b>{item.viewerVoteStatus?vote[item.viewerVoteStatus]||item.viewerVoteStatus:item.separationId?"Times disponíveis":"Aguardando escalação"}</b><p>{item.nextAction?.description||"As novidades aparecerão aqui conforme a rodada avançar."}</p></div></article></div>{Boolean(item.personalHighlights?.length)&&<div className="round-center-personal"><b>✨ Seus destaques</b>{item.personalHighlights!.map(highlight=><span key={highlight}>{highlight}</span>)}</div>}</section>;
}
function weatherCenterLabel(item:MatchHubItem){const weather=item.weatherSummary;if(!weather?.description)return "Previsão indisponível";const temperatures=[weather.temperatureMin,weather.temperatureMax].filter(value=>value!==null).map(value=>`${Math.round(Number(value))}°`);return `${weather.icon||""} ${weather.description}${temperatures.length?` · ${temperatures.join("–")}`:""}`.trim()}
