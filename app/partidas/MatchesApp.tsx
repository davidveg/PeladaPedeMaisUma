...
export default function MatchesApp() {
  ...
  return <div className="member-page"><SiteHeader active="matches" isAdmin={isAdmin}/><main className="member-main match-site-main"><div className="member-account-head"><div><div className="eyebrow">AGENDA DA PELADA</div><h1>Partidas e presenças</h1><p>Suas respostas são sincronizadas entre o site e o aplicativo.</p></div><a className="ghost" href="/notificacoes">🔔 Notificações</a></div>{error && <div className="alert error">{error}</div>}{notice && <div className="admin-notice"><span>✓</span><b>{notice}</b><button onClick={() => setNotice("")}>×</button></div>}<div className="match-site-list">{matches.length ? matches.map(item => <article id={item.id} className={`match-site-card ${item.status.toLowerCase()}`} key={item.id}><div className="match-site-head"><div><span className={`match-state ${item.status.toLowerCase()}`}>{statusLabel(item.status)}</span><h2>{item.title}</h2><p>{dateTime(item.matchAt)}{item.location ? ` · ${item.location}` : ""}</p></div><div className="match-site-count"><b>{item.counts.present}</b><span>confirmados</span></div></div><div className="match-site-deadline">Confirmações até <b>{dateTime(item.confirmationDeadline)}</b> · {item.maxChanges} remarcações permitidas</div><div className="match-site-roster"><Roster title="Presentes" entries={item.attendance.filter(entry => entry.status === "PRESENT")}/><Roster title="Ausentes" entries={item.attendance.filter(entry => entry.status === "ABSENT")}/></div>{item.viewer.playerId ? <div className="match-site-answer"><span>{item.viewer.status ? `Sua resposta: ${item.viewer.status === "PRESENT" ? "Presente" : "Ausente"} · ${item.viewer.changesRemaining} remarcações restantes` : "Você ainda não respondeu."}</span>{item.status === "OPEN" && <div><button disabled={busy === item.id || !item.viewer.canRespond} className={item.viewer.status === "PRESENT" ? "attendance-present on" : "attendance-present"} onClick={() => answer(item, "PRESENT")}>✓ Vou jogar</button><button disabled={busy === item.id || !item.viewer.canRespond} className={item.viewer.status === "ABSENT" ? "attendance-absent on" : "attendance-absent"} onClick={() => answer(item, "ABSENT")}>× Não vou</button></div>}</div> : <div className="alert">Sua conta ainda não está associada a um jogador. Faça a associação em “Minha conta” para responder.</div>}{item.separationId && <a className="ghost match-separation-link" href={`/separacoes-salvas?separation=${encodeURIComponent(item.separationId)}`}>Ver times gerados ↗</a>}</article>) : <div className="empty">Nenhuma partida criada ainda.</div>}</div></main></div>;
}
