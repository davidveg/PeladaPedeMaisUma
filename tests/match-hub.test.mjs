...
test("hub unifica histórico e agenda sem alterar dados, vínculos ou permissões", async t => {
  ...
  try {
    ...
    await t.test("resultado confirmado e contadores compactos respeitam elegibilidade", async () => {
      ...
      const detail = (await get("?match=generated")).items[0];
      assert.equal(detail.confirmationDeadline, future);
      assert.equal(detail.viewerVoteStatus, "AVAILABLE");
      assert.deepEqual(detail.nextAction, { tab: "voting", label: "Votar nos destaques", description: `Votação aberta até ${new Date(future).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo", dateStyle: "short", timeStyle: "short" })}.` });
      ...
      await db().prepare("UPDATE career_matches SET status='CLOSED',results_snapshot=? WHERE id='career'").bind(JSON.stringify({ voteCount: 1, motm: [], dotm: [], partner: [{ playerId: player.id, momentum: .1 }], fairPlay: [], defense: [] })).run();
      const personal = (await get("?match=generated")).items[0];
      assert.deepEqual(personal.personalHighlights, ["Você recebeu o reconhecimento de Parceiro da rodada."]);
      assert.equal(personal.nextAction.tab, "result");
