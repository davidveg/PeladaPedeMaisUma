...
export function CareerVoting({ token, onChanged }: { token: string; onChanged: () => void }) {
  ...
  if (!data.viewer.hasPlayerAssociation) return <VotingNotice title="Associação pendente" message="Sua conta precisa estar associada a um jogador para participar. Solicite a aprovação de um administrador."/>;
