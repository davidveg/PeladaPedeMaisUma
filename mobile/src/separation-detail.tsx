...
function RoundRecap({item,baseUrl}:{item:Separation;baseUrl?:string}){const recap=item.career!.recap!;const link=baseUrl?`${baseUrl.replace(/\/$/,"")}/separacoes-salvas?separation=${encodeURIComponent(item.id)}`:"";return <Card style={{gap:12,backgroundColor:"#FBFDFB"}}><View style={{flexDirection:"row",alignItems:"flex-start",gap:12}}><Text style={{fontSize:28}}>📰</Text><View style={{flex:1,gap:3}}><Text style={{color:colors.green,fontSize:10,fontWeight:"900",letterSpacing:1}}>RESENHA AUTOMÁTICA</Text><Text style={{color:colors.text,fontSize:19,fontWeight:"900"}}>{recap.title}</Text><Text style={{color:colors.green,fontSize:16,fontWeight:"800"}}>{recap.headline}</Text></View></View>{recap.highlights.map((highlight,index)=><View key={index} style={{flexDirection:"row",gap:8,padding:10,borderRadius:10,backgroundColor:"#F0F5F2"}}><Text style={{color:colors.green,fontWeight:"900"}}>•</Text><Text style={{flex:1,color:colors.text,lineHeight:18}}>{highlight}</Text></View>)}<Button title="Compartilhar resenha" icon="whatsapp" disabled={!link} onPress={()=>shareText(`${recap.shareText}\n\n${link}`).catch(error=>Alert.alert("Compartilhamento indisponível",error.message))}/></Card>}
