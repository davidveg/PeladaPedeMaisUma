...
import type { CareerRuleSnapshot, Contribution, Player, Separation, TeamResult } from "@/types";
...
function RoundRecap({item,baseUrl}:{item:Separation;baseUrl?:string}){
  ...
  const shareNewspaper=async()=>{let uri="";setSharingImage(true);try{if(!await Sharing.isAvailableAsync())throw new Error("O compartilhamento de arquivos não está disponível neste aparelho.");uri=await captureRef(newspaperRef,{format:"png",quality:1,result:"tmpfile"});await Clipboard.setStringAsync(`${recap.shareText}\n\n${link}`);await Sharing.shareAsync(uri,{dialogTitle:recap.title,mimeType:"image/png",UTI:"public.png"});Alert.alert("Legenda copiada","A legenda e o link estão na área de transferência. Cole-os na conversa após enviar a imagem.")}catch(error:any){Alert.alert("Compartilhamento indisponível",error?.message||"Não foi possível gerar a imagem do jornal.")}finally{if(uri)releaseCapture(uri);setSharingImage(false)}};
  return <Card style={{gap:0,padding:0,overflow:"hidden",backgroundColor:"#FBF7ED",borderColor:"#D9CFB8"}}>
    ...
    <View style={{padding:18,gap:10,borderTopWidth:3,borderTopColor:"#363228",backgroundColor:"#FFFDF7"}}><Text style={{color:"#27251F",fontFamily:"serif",fontSize:16,fontWeight:"900"}}>Leve a notícia para o grupo.</Text><Text style={{color:"#756C5C",fontSize:12}}>A imagem será aberta para compartilhamento; a legenda e o link ficam copiados para você colar na conversa.</Text><Button title="Compartilhar jornal com imagem" icon="whatsapp" busy={sharingImage} disabled={!link} onPress={shareNewspaper}/></View>
  </Card>;
}
...
function CareerRulesCard({rules}:{rules:CareerRuleSnapshot}){const value=(key:keyof CareerRuleSnapshot,fallback:number)=>Number.isFinite(Number(rules[key]))?Number(rules[key]):fallback,decimal=(number:number,digits=1)=>number.toLocaleString("pt-BR",{minimumFractionDigits:digits,maximumFractionDigits:digits}),percent=(number:number)=>`${decimal(number*100,number*100%1?1:0)}%`,signedValue=(number:number,digits=1)=>`${number>0?"+":""}${decimal(number,digits)}`,resultMultiplier=value("resultMomentumMultiplier",1),votingMultiplier=value("momentumMultiplier",1),winnerBonus=value("winnerBonus",.1),motmFirst=value("motmFirst",.3);return <Card style={{gap:14}}><View style={{gap:4}}><Text style={styles.teamEditorEyebrow}>CONFIGURAÇÃO HISTÓRICA</Text><Text style={styles.teamEditorTitle}>Como o Momentum foi configurado</Text><Text style={styles.teamEditorHelp}>Os pontos abaixo formam saldos de Momentum separados. Cada multiplicador atua somente sobre seu saldo; a contribuição calculada é então somada ao OVR-base.</Text></View><View style={styles.ruleWeights}><RuleWeight label="Momentum de resultados" percent={percent(resultMultiplier)} multiplier={resultMultiplier} note="do saldo de vitórias e derrotas"/><RuleWeight label="Momentum de votações" percent={percent(votingMultiplier)} multiplier={votingMultiplier} note="do saldo obtido nas votações"/></View><Text style={styles.ruleExample}><Text style={{fontWeight:"900"}}>Exemplo: </Text>vitória {signedValue(winnerBonus)} × {decimal(resultMultiplier)} = {signedValue(winnerBonus*resultMultiplier,2)} no OVR; 1º no Man of the Match {signedValue(motmFirst)} × {decimal(votingMultiplier)} = {signedValue(motmFirst*votingMultiplier,2)} no OVR.</Text><Text style={styles.rulePointsLabel}>PONTOS BRUTOS DE MOMENTUM</Text><View style={{gap:8}}><RulePoints title="Resultado da partida" values={`Vitória ${signedValue(winnerBonus)}  ·  Empate 0,0  ·  Derrota ${signedValue(value("loserPenalty",-.1))}`}/><RulePoints title="Man of the Match" values={`1º ${signedValue(motmFirst)}  ·  2º ${signedValue(value("motmSecond",.2))}  ·  3º ${signedValue(value("motmThird",.1))}`}/><RulePoints title="Deception of the Match" values={`1º ${signedValue(value("dotmFirst",-.3))}  ·  2º ${signedValue(value("dotmSecond",-.2))}  ·  3º ${signedValue(value("dotmThird",-.1))}`}/><RulePoints title="Reconhecimentos" values={`Parceiro ${signedValue(value("partnerAward",.1))}  ·  Fair Play ${signedValue(value("fairPlayAward",.1))}  ·  Defesa ${signedValue(value("defenseAward",.1))}`}/></View></Card>}
function RuleWeight({label,percent,multiplier,note}:{label:string;percent:string;multiplier:number;note:string}){return <View style={styles.ruleWeight}><Text style={styles.ruleLabel}>{label.toUpperCase()}</Text><Text style={styles.rulePercent}>{multiplier.toLocaleString("pt-BR",{minimumFractionDigits:1,maximumFractionDigits:1})}×</Text><Text style={styles.ruleNote}>{percent} {note}</Text></View>}
function RulePoints({title,values}:{title:string;values:string}){return <View style={styles.rulePoints}><Text style={{color:colors.text,fontWeight:"900"}}>{title}</Text><Text style={{color:colors.muted,lineHeight:19}}>{values}</Text></View>}

function TeamAssignmentEditor({ item, confirmed = false, onSaved }: { item: Separation; confirmed?: boolean; onSaved: () => void }) {
  const { config: brand, palette } = useMobileBranding();
  const [editing, setEditing] = useState(false), [draft, setDraft] = useState(item.snapshot);
  const mutation = useMutation({
    mutationFn: () => apiFetch<{ snapshot: TeamResult; message: string }>("/api/mobile/separations", jsonMutation("PATCH", { action: confirmed ? "correct-confirmed-teams" : "teams", id: item.id, blue: draft.blue.map(player => player.id), yellow: draft.yellow.map(player => player.id) })),
  ...
  if (!editing) return <Card style={styles.teamEditorIntro}><View style={{ flex: 1, gap: 4 }}><Text style={styles.teamEditorEyebrow}>{confirmed ? "CORREÇÃO DA ÚLTIMA PARTIDA" : "AJUSTE ANTES DO RESULTADO"}</Text><Text style={styles.teamEditorTitle}>Distribuição dos jogadores</Text><Text style={styles.teamEditorHelp}>{confirmed ? "Corrija o lado em que cada jogador realmente atuou. Estatísticas e Momentum serão atualizados sem apagar os votos." : "Transfira jogadores caso um time fique desfalcado. Depois que o resultado for salvo, os times serão bloqueados."}</Text></View><Button title={confirmed ? "Corrigir escalação" : "Editar times"} variant="secondary" onPress={() => setEditing(true)}/></Card>;
  const confirm = () => Alert.alert(confirmed ? "Confirmar correção?" : "Salvar novos times?", confirmed ? "O Momentum do resultado será recalculado. Os votos serão preservados; trocas incompatíveis com gols ou assistências serão bloqueadas." : "A ordem de chegada e um eventual rascunho de súmula serão limpos para evitar inconsistências.", [{ text: "Cancelar", style: "cancel" }, { text: confirmed ? "Confirmar" : "Salvar", style: confirmed ? "destructive" : "default", onPress: () => mutation.mutate() }]);
  return <Card style={styles.teamEditor}><Text style={styles.teamEditorTitle}>Editar distribuição</Text><Text style={styles.teamEditorHelp}>Use a seta ao lado do jogador para transferi-lo ao outro time.</Text><EditableTeam title={brand.teamBlueName} color={palette.blue} soft={palette.blueSoft} direction="→" players={draft.blue} onMove={id => move("blue", id)}/><EditableTeam title={brand.teamYellowName} color={palette.yellow} soft={palette.yellowSoft} direction="←" players={draft.yellow} onMove={id => move("yellow", id)}/><BalanceDetails result={draft}/><View style={styles.teamEditorActions}><Button title="Cancelar" variant="secondary" disabled={mutation.isPending} onPress={() => { setDraft(item.snapshot); setEditing(false); }}/><Button title={confirmed ? "Confirmar correção" : "Salvar times"} busy={mutation.isPending} onPress={confirm}/></View></Card>;
}
...
const styles = {
  ...
  ruleWeights:{flexDirection:"row" as const,flexWrap:"wrap" as const,gap:9},
  ruleWeight:{flexGrow:1,flexBasis:135,minHeight:92,padding:13,borderRadius:12,backgroundColor:"#EEF5F1",justifyContent:"center" as const,gap:3},
  ruleLabel:{color:colors.green,fontSize:9,fontWeight:"900" as const,letterSpacing:.5},
  rulePercent:{color:colors.green,fontSize:27,fontWeight:"900" as const},
  ruleNote:{color:colors.muted,fontSize:11},
  ruleExample:{padding:11,borderLeftWidth:3,borderLeftColor:colors.green,borderRadius:8,backgroundColor:"#F5F8F6",color:colors.muted,fontSize:12,lineHeight:18},
  rulePointsLabel:{color:colors.green,fontSize:9,fontWeight:"900" as const,letterSpacing:.7},
  rulePoints:{padding:12,borderWidth:1,borderColor:colors.border,borderRadius:11,backgroundColor:colors.cream,gap:4},
};
