...
export type Player = { id: string; displayName: string; fullName?: string; nickname?: string | null; aliases?: string[]; type: string; primaryPosition: string; speed: number; skill: number; marking?: number; goalkeeperPositioning?: number; goalExit?: number; momentum?: number; resultMomentum?: number; votingMomentum?: number; photoUrl?: string | null; notes?: string | null; careerStats?: CareerStats };
...
export type TeamMetrics = { count: number; positions: { Defesa: number; "Meio-campo": number; Ataque: number; Goleiro: number }; speed: number; skill: number; marking: number; momentum: number; total: number; speedAvg: number; skillAvg: number; markingAvg: number; momentumAvg: number; scoreAvg: number };
export type TeamDelta = { players: number; defenders: number; midfielders: number; attackers: number; speed: number; skill: number; marking: number; momentum: number; score: number };
export type TeamResult = { blue: Player[]; yellow: Player[]; rating: string; cost: number; blueMetrics?: TeamMetrics; yellowMetrics?: TeamMetrics; delta?: TeamDelta; speedWeight: number; skillWeight: number; markingWeight: number; resultMomentumMultiplier?: number; momentumMultiplier?: number; maximumPositionDifference?: number; protectedTopPlayersPercentage?: number; algorithmAttempts?: number; proposal?: number; extraId?: string; [key: string]: unknown };
...
export type ProfilePayload = { member: { id: string; email: string; accountType: "administrator" | "member"; playerId?: string | null }; player: Player | null; config?: { speedWeight: number; skillWeight: number; markingWeight: number; resultMomentumMultiplier: number; momentumMultiplier: number; showContributions: boolean; cardTiersEnabled: boolean; cardBronzeMax: number; cardSilverMax: number; cardGoldMax: number } };
