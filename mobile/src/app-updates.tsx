...
export function AppUpdateCoordinator() {
  ...
  return <Modal transparent visible animationType="fade" statusBarTranslucent onRequestClose={release.required ? () => undefined : dismiss}>
    <View style={[styles.backdrop,{paddingTop:Math.max(16,insets.top+8),paddingBottom:Math.max(16,insets.bottom+8)}]}><View accessibilityViewIsModal style={styles.card}>
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent} persistentScrollbar showsVerticalScrollIndicator>
        <View style={styles.icon}><Text style={styles.iconText}>↑</Text></View><Text accessibilityRole="header" style={styles.title}>{release.required ? "Atualização necessária" : "Nova versão disponível"}</Text>
...
const styles = StyleSheet.create({
  backdrop: { flex: 1, justifyContent: "center", paddingHorizontal: 16, backgroundColor: "rgba(5,26,20,.72)" },
  card: { width: "100%", maxWidth: 460, maxHeight: "92%", alignSelf: "center", overflow: "hidden", borderRadius: 22, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.cream, ...shadow },
  body: { flexShrink: 1 },
  bodyContent: { paddingHorizontal: 22, paddingTop: 22, paddingBottom: 14 },
  ...
  actions: { flexShrink: 0, borderTopWidth: 1, borderTopColor: colors.border, paddingHorizontal: 22, paddingBottom: 18, backgroundColor: colors.cream },
  current: { color: colors.muted, fontSize: 11, textAlign: "center", marginVertical: 12 },
