...
export default function Login() {
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: palette.green }}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, justifyContent: "center", padding: 24 }}><View style={{ alignItems: "center", marginBottom: 32 }}><Text style={{ fontSize: 50 }}>⚽</Text><Text style={{ color: "#fff", fontSize: 31, fontWeight: "900", textAlign: "center" }}>{brand.appName}</Text><Text style={{ color: brand.appSecondaryColor, fontSize: 18, fontWeight: "700", textAlign: "center" }}>{brand.appTagline}</Text></View><View style={{ backgroundColor: palette.cream, padding: 20, borderRadius: 22, gap: 16 }}><Text accessibilityRole="header" style={{ fontSize: 24, fontWeight: "800", color: palette.text }}>Entre para a partida</Text><Controller control={control} name="email" render={({ field }) => <Field label="E-mail" autoCapitalize="none" keyboardType="email-address" autoComplete="email" value={field.value} onChangeText={field.onChange} onBlur={field.onBlur} error={errors.email?.message}/>}/><Controller control={control} name="password" render={({ field }) => <Field label="Senha" secureTextEntry={!visible} autoComplete="current-password" value={field.value} onChangeText={field.onChange} onBlur={field.onBlur} error={errors.password?.message}/>}/><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}><Pressable accessibilityRole="button" onPress={() => setVisible(value => !value)} style={{ minHeight: 44, justifyContent: "center" }}><Text style={{ color: palette.green, fontWeight: "700" }}>{visible ? "Ocultar senha" : "Mostrar senha"}</Text></Pressable><Pressable accessibilityRole="button" onPress={() => setRecovery(true)} style={{ minHeight: 44, justifyContent: "center" }}><Text style={{ color: palette.green, fontWeight: "800" }}>Esqueci minha senha</Text></Pressable></View>{message ? <Text accessibilityRole="alert" style={{ color: colors.danger, fontWeight: "700" }}>{message}</Text> : null}<Button title="Entrar" busy={isSubmitting} onPress={submit}/><Pressable onPress={() => API_BASE_URL && Linking.openURL(`${API_BASE_URL}/conta`)} style={{ minHeight: 44, justifyContent: "center", alignItems: "center" }}><Text style={{ color: palette.green }}>Primeiro acesso? Use a aplicação web.</Text></Pressable></View><Text style={{ color: brand.appSecondaryColor, textAlign: "center", fontSize: 12, marginTop: 20 }}>Versão {Application.nativeApplicationVersion || "desenvolvimento"} · {process.env.EXPO_PUBLIC_APP_ENV || "local"}\n{API_BASE_URL || "Servidor não configurado"}</Text></ScrollView><PasswordRecoveryModal visible={recovery} onClose={() => setRecovery(false)}/></KeyboardAvoidingView>;
}
