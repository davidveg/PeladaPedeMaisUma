...
export default function MatchDetail() {
  ...
  return <Screen><FlatList
    ...
    ListHeaderComponent={<><Header eyebrow={item.status === "OPEN" ? item.acceptingResponses ? "CONFIRMAÇÕES ABERTAS" : "PRAZO ENCERRADO" : item.status === "CLOSED" ? "LISTA ENCERRADA" : "PARTIDA CANCELADA"} title={item.title}/>
      ...
      {account?.role === "admin" && item.status === "OPEN" ? <Button title="Editar data e regras" variant="secondary" onPress={() => router.push(`/matches/manage?id=${item.id}` as never)}/> : null}
      {item.status === "OPEN" && item.shareMessage ? <Button title="Compartilhar parcial no WhatsApp" variant="secondary" onPress={() => shareText(item.shareMessage).catch(error => Alert.alert("Compartilhamento indisponível", error.message))}/> : null}
...
const styles = StyleSheet.create({
  ...
  footer: { gap: 10, marginTop: 8 }, smallDisabled: { opacity: .35 },
});
