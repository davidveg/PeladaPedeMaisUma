...
function MatchForm({ existing, onSaved, onCancel }: { existing?: ScheduledMatch; onSaved(): Promise<void>; onCancel(): void }) {
  const [defaults] = useState(nextSundayDefaults);
  ...
  const [title, setTitle] = useState(existing?.title || "Pelada"), [location, setLocation] = useState(existing?.location || "");
