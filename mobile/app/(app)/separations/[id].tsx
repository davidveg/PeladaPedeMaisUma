...
export default function SeparationDetail() {
  ...
  return <Screen><Header eyebrow={formatDate(item.matchDate || item.confirmedAt)} title={item.matchTitle}/><ScrollView refreshControl={<RefreshControl refreshing={listQuery.isRefetching} onRefresh={listQuery.refetch} tintColor={colors.green}/>} contentContainerStyle={{ padding: 20, paddingTop: 8, gap: 14 }}>
    ...
    {admin ? <><Button title="Compartilhar times no WhatsApp" icon="whatsapp" variant="secondary" disabled={!publicQuery.data?.baseUrl} onPress={() => publicQuery.data?.baseUrl && shareText(separationMessage(item, publicQuery.data.baseUrl)).catch(error => Alert.alert("Compartilhamento indisponível", error.message))}/><ArrivalEditor item={item} onSaved={() => client.invalidateQueries({ queryKey: ["separations"] })}/><MatchPanel item={item} onSaved={() => client.invalidateQueries({ queryKey: ["separations"] })}/>{item.career?.status === "CLOSED" ? <Button title="Compartilhar resultado no WhatsApp" icon="whatsapp" disabled={!publicQuery.data?.baseUrl} onPress={() => publicQuery.data?.baseUrl && shareText(careerResultsMessage(item, publicQuery.data.baseUrl, { siteName: brand.appName, teamBlueName: brand.teamBlueName, teamYellowName: brand.teamYellowName })).catch(error => Alert.alert("Compartilhamento indisponível", error.message))}/> : item.career?.votingUrl ? <Button title="Compartilhar votação no WhatsApp" icon="whatsapp" onPress={() => shareText(votingMessage(item, item.career!.votingUrl!)).catch(error => Alert.alert("Compartilhamento indisponível", error.message))}/> : null}</> : null}
  </ScrollView></Screen>;
}
