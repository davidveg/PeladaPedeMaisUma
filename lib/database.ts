...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS login_rate_limits (id TEXT PRIMARY KEY, failures INTEGER NOT NULL DEFAULT 0, window_started_at TEXT NOT NULL, blocked_until TEXT, updated_at TEXT NOT NULL)`,
  `CREATE INDEX IF NOT EXISTS login_rate_limits_updated_idx ON login_rate_limits(updated_at)`,
  `CREATE TABLE IF NOT EXISTS upload_objects (object_key TEXT PRIMARY KEY, purpose TEXT NOT NULL CHECK(purpose IN ('players','branding')), owner_account_type TEXT NOT NULL CHECK(owner_account_type IN ('administrator','member')), owner_account_id TEXT NOT NULL, size_bytes INTEGER NOT NULL DEFAULT 0, content_type TEXT, status TEXT NOT NULL CHECK(status IN ('uploading','pending','attached','rejected','deleted')), attached_entity_type TEXT, attached_entity_id TEXT, created_at TEXT NOT NULL, attached_at TEXT, updated_at TEXT NOT NULL)`,
  ...
  `CREATE TABLE IF NOT EXISTS player_absence_periods (id TEXT PRIMARY KEY, player_id TEXT NOT NULL UNIQUE, start_date TEXT NOT NULL, end_date TEXT NOT NULL, reason TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)`,
  `CREATE INDEX IF NOT EXISTS player_absence_periods_dates_idx ON player_absence_periods(start_date,end_date)`,
  `CREATE TABLE IF NOT EXISTS match_attendance (id TEXT PRIMARY KEY, match_id TEXT NOT NULL, player_id TEXT NOT NULL, status TEXT NOT NULL CHECK(status IN ('PRESENT','ABSENT')), change_count INTEGER NOT NULL DEFAULT 0, responded_by_account_type TEXT, responded_by_account_id TEXT, updated_by_administrator_id TEXT, absence_period_id TEXT, absence_previous_status TEXT, absence_previous_change_count INTEGER, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, UNIQUE(match_id,player_id))`,
  ...
  `CREATE INDEX IF NOT EXISTS team_separations_statistics_date_idx ON team_separations(match_date,deleted_at)`,
  `CREATE TRIGGER IF NOT EXISTS scheduled_matches_delete_cascade BEFORE DELETE ON scheduled_matches BEGIN DELETE FROM match_attendance WHERE match_id=OLD.id; DELETE FROM match_guest_preconfirmations WHERE match_id=OLD.id; DELETE FROM match_separation_drafts WHERE match_id=OLD.id; DELETE FROM account_notifications WHERE match_id=OLD.id; UPDATE team_separations SET deleted_at=COALESCE(deleted_at,strftime('%Y-%m-%dT%H:%M:%fZ','now')),updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=OLD.separation_id; END`,
  ...
  `CREATE TABLE IF NOT EXISTS instance_configuration (id INTEGER PRIMARY KEY, site_name TEXT NOT NULL DEFAULT 'Pelada Pede Mais Uma', site_short_name TEXT NOT NULL DEFAULT 'Pelada', site_tagline TEXT NOT NULL DEFAULT 'Times equilibrados. Resenha garantida.', footer_text TEXT NOT NULL DEFAULT 'Times equilibrados. Resenha garantida.', logo_url TEXT, favicon_url TEXT, share_image_url TEXT, public_logo_size INTEGER NOT NULL DEFAULT 44, admin_logo_size INTEGER NOT NULL DEFAULT 54, show_public_brand_text INTEGER NOT NULL DEFAULT 1, show_admin_brand_text INTEGER NOT NULL DEFAULT 1, primary_color TEXT NOT NULL DEFAULT '#174D3B', admin_sidebar_color TEXT NOT NULL DEFAULT '#133F31', secondary_color TEXT NOT NULL DEFAULT '#D9F36B', background_color TEXT NOT NULL DEFAULT '#F5F7F3', surface_color TEXT NOT NULL DEFAULT '#FFFFFF', text_color TEXT NOT NULL DEFAULT '#15241F', muted_color TEXT NOT NULL DEFAULT '#68756F', team_blue_color TEXT NOT NULL DEFAULT '#1768E5', team_yellow_color TEXT NOT NULL DEFAULT '#F4BF20', team_blue_name TEXT NOT NULL DEFAULT 'Azul', team_yellow_name TEXT NOT NULL DEFAULT 'Amarelo', app_name TEXT NOT NULL DEFAULT 'Pelada Pede Mais Uma', app_tagline TEXT NOT NULL DEFAULT 'Entre para a partida', app_primary_color TEXT NOT NULL DEFAULT '#0B3D2E', app_secondary_color TEXT NOT NULL DEFAULT '#D9F36B', app_background_color TEXT NOT NULL DEFAULT '#F6F4EC', app_text_color TEXT NOT NULL DEFAULT '#17221D', default_match_title TEXT NOT NULL DEFAULT 'Pelada', default_match_weekday INTEGER NOT NULL DEFAULT 0, default_match_time TEXT NOT NULL DEFAULT '09:00', default_match_location TEXT NOT NULL DEFAULT 'Rio de Janeiro, Brasil', confirmation_lead_minutes INTEGER NOT NULL DEFAULT 60, manual_separation_enabled INTEGER NOT NULL DEFAULT 0, separation_drafts_enabled INTEGER NOT NULL DEFAULT 0, guest_preconfirmation_enabled INTEGER NOT NULL DEFAULT 0, guest_confirmation_threshold INTEGER NOT NULL DEFAULT 16, finance_enabled INTEGER NOT NULL DEFAULT 1, delinquency_attendance_block_enabled INTEGER NOT NULL DEFAULT 0, timezone TEXT NOT NULL DEFAULT 'America/Sao_Paulo', updated_at TEXT NOT NULL)`,
...
export async function ensureDb() {
  ...
  if (!ready) ready=(async()=>{
    ...
    const migratedDelinquencyAttendanceBlock=!instanceColumns.results.some((column:any)=>column.name==="delinquency_attendance_block_enabled");
    if(migratedDelinquencyAttendanceBlock) await d.prepare(`ALTER TABLE instance_configuration ADD COLUMN delinquency_attendance_block_enabled INTEGER NOT NULL DEFAULT 0`).run();
    ...
    const migratedAdminSidebarColor=!instanceColumns.results.some((column:any)=>column.name==="admin_sidebar_color");
    if(migratedAdminSidebarColor) await d.prepare(`ALTER TABLE instance_configuration ADD COLUMN admin_sidebar_color TEXT NOT NULL DEFAULT '#133F31'`).run();
    const migratedPublicLogoSize=!instanceColumns.results.some((column:any)=>column.name==="public_logo_size");
    if(migratedPublicLogoSize) await d.prepare(`ALTER TABLE instance_configuration ADD COLUMN public_logo_size INTEGER NOT NULL DEFAULT 44`).run();
    const migratedAdminLogoSize=!instanceColumns.results.some((column:any)=>column.name==="admin_logo_size");
    if(migratedAdminLogoSize) await d.prepare(`ALTER TABLE instance_configuration ADD COLUMN admin_logo_size INTEGER NOT NULL DEFAULT 54`).run();
    const migratedShowPublicBrandText=!instanceColumns.results.some((column:any)=>column.name==="show_public_brand_text");
    if(migratedShowPublicBrandText) await d.prepare(`ALTER TABLE instance_configuration ADD COLUMN show_public_brand_text INTEGER NOT NULL DEFAULT 1`).run();
    const migratedShowAdminBrandText=!instanceColumns.results.some((column:any)=>column.name==="show_admin_brand_text");
    if(migratedShowAdminBrandText) await d.prepare(`ALTER TABLE instance_configuration ADD COLUMN show_admin_brand_text INTEGER NOT NULL DEFAULT 1`).run();
    ...
    const attendanceColumns=await d.prepare(`PRAGMA table_info(match_attendance)`).all();
    if(!attendanceColumns.results.some((column:any)=>column.name==="absence_period_id")) await d.prepare(`ALTER TABLE match_attendance ADD COLUMN absence_period_id TEXT`).run();
    if(!attendanceColumns.results.some((column:any)=>column.name==="absence_previous_status")) await d.prepare(`ALTER TABLE match_attendance ADD COLUMN absence_previous_status TEXT`).run();
    if(!attendanceColumns.results.some((column:any)=>column.name==="absence_previous_change_count")) await d.prepare(`ALTER TABLE match_attendance ADD COLUMN absence_previous_change_count INTEGER`).run();
    ...
    const socialAwardColumns=[["partner_award",.1],["fair_play_award",.1],["defense_award",.1]] as const;
    const migratedSocialAwards=socialAwardColumns.some(([column])=>!careerColumns.results.some((item:any)=>item.name===column));
    for(const [column,value] of socialAwardColumns) if(!careerColumns.results.some((item:any)=>item.name===column)) await d.prepare(`ALTER TABLE career_configuration ADD COLUMN ${column} REAL NOT NULL DEFAULT ${value}`).run();
    const contributionColumns=await d.prepare(`PRAGMA table_info(career_match_contributions)`).all();
    const careerMatchColumns=await d.prepare(`PRAGMA table_info(career_matches)`).all();
    const migratedParticipationSnapshot=!careerMatchColumns.results.some((column:any)=>column.name==="participation_snapshot");
    if(migratedParticipationSnapshot) await d.prepare(`ALTER TABLE career_matches ADD COLUMN participation_snapshot TEXT`).run();
    ...
    const socialVoteColumns=["partner_id","fair_play_id","defense_id"] as const;
    const migratedSocialVotes=socialVoteColumns.some(column=>!voteColumns.results.some((item:any)=>item.name===column));
    for(const column of socialVoteColumns) if(!voteColumns.results.some((item:any)=>item.name===column)) await d.prepare(`ALTER TABLE career_votes ADD COLUMN ${column} TEXT`).run();
    ...
    await seed(d,now);
    logEvent("info","database_ready",{migratedPromotedFromMember,migratedMemberRole,migratedPlayerMarking,migratedPlayerMomentum,migratedSecondaryPosition,migratedResultMomentum,migratedVotingMomentum,migratedGoalkeeperPositioning,migratedGoalExit,migratedMarkingWeight,migratedHistoricalLearning,migratedTeamBlueName,migratedTeamYellowName,migratedManualSeparationEnabled,migratedSeparationDraftsEnabled,migratedGuestPreconfirmationEnabled,migratedGuestConfirmationThreshold,migratedFinanceEnabled,migratedShareImageUrl,migratedFaviconUrl,migratedAdminSidebarColor,migratedPublicLogoSize,migratedAdminLogoSize,migratedShowPublicBrandText,migratedShowAdminBrandText,migratedMomentumMultiplier,migratedResultMomentumMultiplier,migratedSeasonDuration,migratedSeasonStartedAt,migratedNextSeasonResetAt,migratedSeasonNumber,migratedTrackContributions,migratedCardTiers,migratedCardBronzeMax,migratedCardSilverMax,migratedCardGoldMax,migratedMonthlyTeamFormation,migratedSocialAwards,migratedOwnGoal,migratedArrivalOrder,migratedMatchDraft,migratedParticipationSnapshot,migratedVoteAccountType,migratedVoteAccountId,migratedSocialVotes,migratedNotificationActionUrl,migratedAppUpdatesInApp,migratedAppUpdatesPush});
  })();
  return ready;
}
...
export async function currentAdmin(request:Request){await ensureDb();const token=(request.headers.get("cookie")||"").match(/ppm_session=([^;]+)/)?.[1];if(token){const storedToken=await migrateWebSessionToken("sessions",token);return db().prepare(`SELECT a.id,a.email,a.active,a.must_change_password mustChangePassword,a.created_at createdAt,l.player_id playerId,'administrator' accountType,'administrator' role FROM sessions s JOIN administrators a ON a.id=s.administrator_id LEFT JOIN player_account_links l ON l.account_type='administrator' AND l.account_id=a.id WHERE s.id=? AND s.expires_at>? AND a.active=1`).bind(storedToken,new Date().toISOString()).first()}const mobile=await currentMobileAccount(request);return mobile?.accountType==="administrator"?mobile:null;}
export async function adminRequired(request:Request){const administrator:any=await currentAdmin(request);return administrator&&!administrator.mustChangePassword?administrator:null}
export async function currentMember(request:Request){await ensureDb();const token=(request.headers.get("cookie")||"").match(/ppm_member_session=([^;]+)/)?.[1];if(token){const storedToken=await migrateWebSessionToken("member_sessions",token);const member=await db().prepare(`SELECT a.id,a.email,a.role,l.player_id playerId,a.created_at createdAt,'member' accountType FROM member_sessions s JOIN member_accounts a ON a.id=s.member_account_id LEFT JOIN player_account_links l ON l.account_type='member' AND l.account_id=a.id WHERE s.id=? AND s.expires_at>? AND a.active=1`).bind(storedToken,new Date().toISOString()).first();return withModeratorPermissions(member)}const mobile=await currentMobileAccount(request);return mobile?.accountType==="member"?withModeratorPermissions(mobile):null;}
...
export async function staffRequired(request:Request,permission:ModeratorPermission){const account:any=await currentStaff(request);if(!account||account.accountType==="administrator"&&account.mustChangePassword)return null;if(account.accountType==="administrator")return account;return Array.isArray(account.permissions)&&account.permissions.includes(permission)?account:null}
export async function staffRequiredAny(request:Request,permissions:ModeratorPermission[]){const account:any=await currentStaff(request);if(!account||account.accountType==="administrator"&&account.mustChangePassword)return null;if(account.accountType==="administrator")return account;return Array.isArray(account.permissions)&&permissions.some(permission=>account.permissions.includes(permission))?account:null}
...
async function migrateWebSessionToken(table:"sessions"|"member_sessions",token:string){const hash=await hashOpaqueToken(token),now=new Date().toISOString();const legacy=await db().prepare(`SELECT id FROM ${table} WHERE id IN (?,?) AND expires_at>? LIMIT 1`).bind(hash,token,now).first<{id:string}>();if(legacy?.id===token)await db().prepare(`UPDATE ${table} SET id=? WHERE id=?`).bind(hash,token).run();return hash}
