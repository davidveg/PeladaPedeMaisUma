...
export async function audit(adminId:string|null,action:string,entityType:string,entityId?:string,data?:unknown){await db().prepare(`INSERT INTO audit_logs VALUES (?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),adminId,action,entityType,entityId||null,null,data?JSON.stringify(data):null,new Date().toISOString()).run()}
