...
export async function ensureDb() {
  if (!ready) ready=(async()=>{
    ...
    await seed(d,now);
    logEvent("info","database_ready",{migratedPlayerMarking,migratedPlayerMomentum,migratedGoalkeeperPositioning,migratedGoalExit,migratedMarkingWeight,migratedTeamBlueName,migratedTeamYellowName,migratedMomentumMultiplier,migratedTrackContributions,migratedCardTiers,migratedCardBronzeMax,migratedCardSilverMax,migratedCardGoldMax,migratedOwnGoal,migratedArrivalOrder,migratedMatchDraft,migratedVoteAccountType,migratedVoteAccountId,migratedNotificationActionUrl,migratedAppUpdatesInApp,migratedAppUpdatesPush});
  })();
  return ready;
}
