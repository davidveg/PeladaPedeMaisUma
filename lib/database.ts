...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS instance_configuration (id INTEGER PRIMARY KEY, site_name TEXT NOT NULL DEFAULT 'Pelada Pede Mais Uma', site_short_name TEXT NOT NULL DEFAULT 'Pelada', site_tagline TEXT NOT NULL DEFAULT 'Times equilibrados. Resenha garantida.', footer_text TEXT NOT NULL DEFAULT 'Times equilibrados. Resenha garantida.', logo_url TEXT, primary_color TEXT NOT NULL DEFAULT '#174D3B', secondary_color TEXT NOT NULL DEFAULT '#D9F36B', background_color TEXT NOT NULL DEFAULT '#F5F7F3', surface_color TEXT NOT NULL DEFAULT '#FFFFFF', text_color TEXT NOT NULL DEFAULT '#15241F', muted_color TEXT NOT NULL DEFAULT '#68756F', team_blue_color TEXT NOT NULL DEFAULT '#1768E5', team_yellow_color TEXT NOT NULL DEFAULT '#F4BF20', app_name TEXT NOT NULL DEFAULT 'Pelada Pede Mais Uma', app_tagline TEXT NOT NULL DEFAULT 'Entre para a partida', app_primary_color TEXT NOT NULL DEFAULT '#0B3D2E', app_secondary_color TEXT NOT NULL DEFAULT '#D9F36B', app_background_color TEXT NOT NULL DEFAULT '#F6F4EC', app_text_color TEXT NOT NULL DEFAULT '#17221D', default_match_title TEXT NOT NULL DEFAULT 'Pelada', default_match_weekday INTEGER NOT NULL DEFAULT 0, default_match_time TEXT NOT NULL DEFAULT '09:00', confirmation_lead_minutes INTEGER NOT NULL DEFAULT 60, timezone TEXT NOT NULL DEFAULT 'America/Sao_Paulo', updated_at TEXT NOT NULL)`,
...
export async function ensureDb() {
  if (!ready) ready=(async()=>{
    ...
    await seed(d,now);
    logEvent("info","database_ready",{migratedPlayerMarking,migratedPlayerMomentum,migratedGoalkeeperPositioning,migratedGoalExit,migratedMarkingWeight,migratedMomentumMultiplier,migratedTrackContributions,migratedCardTiers,migratedCardBronzeMax,migratedCardSilverMax,migratedCardGoldMax,migratedOwnGoal,migratedArrivalOrder,migratedMatchDraft,migratedVoteAccountType,migratedVoteAccountId,migratedNotificationActionUrl,migratedAppUpdatesInApp,migratedAppUpdatesPush});
  })();
  return ready;
}
