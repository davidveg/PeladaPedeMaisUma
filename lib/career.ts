...
export function validateCareerConfig(config: CareerConfig) {
  const positive = [config.winnerBonus, config.motmThird, config.motmSecond, config.motmFirst, config.partnerAward, config.fairPlayAward, config.defenseAward];
...
export type CareerVoteInput = { voterPlayerId: string; motmThirdId: string; motmSecondId: string; motmFirstId: string; dotmThirdId: string; dotmSecondId: string; dotmFirstId: string; partnerId?: string; fairPlayId?: string; defenseId?: string };
...
export function careerVoteForAuthenticatedPlayer(payload: Record<string, unknown>, voterPlayerId: string): CareerVoteInput {
  return {
    ...
    fairPlayId: String(payload.fairPlayId || ""),
    defenseId: String(payload.defenseId || ""),
  };
}
...
export function validateCareerVote(input: CareerVoteInput, participantIds: string[]) {
  ...
  const recognitions = [input.partnerId, input.fairPlayId, input.defenseId].filter(Boolean) as string[];
  const participants = new Set(participantIds);
  if (!participants.has(input.voterPlayerId)) return "O jogador identificado não participou desta partida.";
  if (recognitions.length > 0 && recognitions.length < 3) return "Preencha os três reconhecimentos da rodada.";
  if ([...podium, ...recognitions].some(id => !participants.has(id))) return "Todos os votos devem ser destinados a jogadores desta partida.";
  if ([...podium, ...recognitions].includes(input.voterPlayerId)) return "O jogador não pode votar em si mesmo.";
...
export function rankCareerRecognition(votes: any[], field: "partner_id" | "fair_play_id" | "defense_id") {
  const totals = new Map<string, number>();
  for (const vote of votes) {
    const playerId = String(vote[field] || "");
    if (playerId) totals.set(playerId, (totals.get(playerId) || 0) + 1);
  }
  const winner = [...totals].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0];
  return winner ? [{ playerId: winner[0], votes: winner[1], place: 1 }] : [];
}
